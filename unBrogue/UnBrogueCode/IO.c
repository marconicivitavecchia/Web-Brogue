...
void mainInputLoop() {
	...
	boolean canceled, targetConfirmed, tabKey, focusedOnMonster, focusedOnItem,
	...
	while (!rogue.gameHasEnded && (!playingBack || !canceled)) { // repeats until the game ends
		...
		fillDynamicGrid(playerPathingMap, 30000);
		...
		do {
			...
			if (coordinatesAreInMap(cursor[0], cursor[1])) {
				hiliteCell(cursor[0],
						   ...
						   ((path[steps-1][0] == cursor[0] && path[steps-1][1] == cursor[1])
							|| (!playingBack && distanceDiagonal(player.xLoc, player.yLoc, cursor[0], cursor[1]) <= 1) ? 100 : 25),
						   true);
			...
			if (tabKey && !playingBack) { // The tab key cycles the cursor through monsters and items.
				if (nextTargetAfter(&newX, &newY, cursor[0], cursor[1], true, true, true, false)) {
                    cursor[0] = newX;
                    cursor[1] = newY;
                }
			}
			...
			if (theEvent.eventType == MOUSE_UP
				...
				if (originLoc[0] == cursor[0]
					...
					confirmMessages();
						   ...
						   || (distanceDiagonal(player.xLoc, player.yLoc, cursor[0], cursor[1]) == 1 // includes diagonals
							   && (!diagonalBlocked(player.xLoc, player.yLoc, cursor[0], cursor[1])
                                   ...
                                   || ((terrainFlags(cursor[0], cursor[1]) & T_OBSTRUCTS_PASSABILITY) && (terrainFlags(cursor[0], cursor[1]) & T_PROMOTES_ON_PLAYER_ENTRY))))) { // there's a lever there
...
void getCellAppearance(short x, short y, uchar *returnChar, color *returnForeColor, color *returnBackColor) {
	...
	uchar itemChars[] = {POTION_CHAR, ELIXIR_CHAR, SCROLL_CHAR, TOME_CHAR, FOOD_CHAR, WAND_CHAR, TALISMAN_CHAR,
						STAFF_CHAR, GOLD_CHAR, ARMOR_CHAR, WEAPON_CHAR, SHIELD_CHAR, CHARM_CHAR, RING_CHAR};
	...
	if (!playerCanSeeOrSense(x, y)
		...
		&& (!(player.status[STATUS_TELEPATHIC] || rogue.telepathyBonus >= distanceFastApproximation(x, y, player.xLoc, player.yLoc))
		...
		for (layer = 0; layer < maxLayer; layer++) {
			...
			if (pmap[x][y].layers[layer] && layer != GAS) {
                ...
                if (rogue.playbackOmniscience && (tileCatalog[tile].flags & T_IS_SECRET)) {
		...
		if (pmap[x][y].flags & HAS_PLAYER) {
		...
		} else if ((player.status[STATUS_TELEPATHIC] || rogue.telepathyBonus >= distanceFastApproximation(x, y, player.xLoc, player.yLoc))
		...
		} else if (rogue.impaleDirection >= 0 && (x == player.xLoc + nbDirs[rogue.impaleDirection][0])
				   && (y == player.yLoc + nbDirs[rogue.impaleDirection][1])) {
			cellChar = impaleDirs[rogue.impaleDirection];
			cellForeColor = rogue.weaponBloodstainColor;
			needDistinctness = true;
			...
			needDistinctness = true;
			theItem = itemAtLoc(x, y);
			// Have to duplicate the if statements below, because we've run out of flags for a HAS_DORMANT_ITEM
			if (theItem->flags & ITEM_DORMANT) {
				if (playerCanSeeOrSense(x, y) || (pmap[x][y].flags & (DISCOVERED | MAGIC_MAPPED))) {
				...
				cellChar = itemChars[rand_range(0, 13)];
				cellForeColor = itemColor;
			} else {
		...
		if (gasAugmentWeight) {
			...
			if ((pmap[x][y].flags & HAS_MONSTER)
				...
				&& (playerCanSeeOrSense(x, y) || !(player.status[STATUS_TELEPATHIC] || rogue.telepathyBonus >= distanceFastApproximation(x, y, player.xLoc, player.yLoc))
					|| (monst->info.flags & MONST_INANIMATE))) {
		...
		if (!(pmap[x][y].flags & (ANY_KIND_OF_VISIBLE | ITEM_DETECTED | HAS_PLAYER))
			&& !playerCanSeeOrSense(x, y)
			&& (!(player.status[STATUS_TELEPATHIC] || rogue.telepathyBonus >= distanceFastApproximation(x, y, player.xLoc, player.yLoc))
				|| !monst || (monst->info.flags & MONST_INANIMATE)) && !monsterWithDetectedItem) {
	...
	if (((pmap[x][y].flags & ITEM_DETECTED) || monsterWithDetectedItem
		 || ((player.status[STATUS_TELEPATHIC] || rogue.telepathyBonus >= distanceFastApproximation(x, y, player.xLoc, player.yLoc))
			 && (pmap[x][y].flags & (HAS_MONSTER | HAS_DORMANT_MONSTER))
			 && monst && !(monst->info.flags & MONST_INANIMATE))
		 || ((pmap[x][y].flags & HAS_MONSTER) && monst && monst->status[STATUS_MARKED]))
		&& !playerCanSeeOrSense(x, y)) {
		// do nothing
	...
	} else if (!(pmap[x][y].flags & VISIBLE) && (pmap[x][y].flags & MARKED_VISIBLE)) {
		...
		if (!rogue.trueColorMode || !needDistinctness) {
			applyColorMultiplier(&cellForeColor, &lightMultiplierColor);
			applyColorMultiplier(&cellForeColor, &markedMultiplier);
		}
		applyColorMultiplier(&cellBackColor, &lightMultiplierColor);
		applyColorMultiplier(&cellBackColor, &markedMultiplier);
...
void applyColorScalar(color *baseColor, short scalar) {
	baseColor->red *= scalar / 100;
	baseColor->redRand *= scalar / 100;
	baseColor->green *= scalar / 100;
	baseColor->greenRand *= scalar / 100;
	baseColor->blue *= scalar / 100;
	baseColor->blueRand *= scalar / 100;
	baseColor->rand *= scalar / 100;
}
...
void colorMultiplierFromDungeonLight(short x, short y, color *editColor) {
	...
	editColor->red		= editColor->redRand	= max(0, tmap[x][y].light[0]);
	editColor->green	= editColor->greenRand	= max(0, tmap[x][y].light[1]);
	editColor->blue		= editColor->blueRand	= max(0, tmap[x][y].light[2]);
...
// Highlight the portion indicated by hiliteGrid with the hiliteColor at the hiliteStrength -- both latter arguments are optional.
...
void funkyFade(cellDisplayBuffer displayBuf[COLS][ROWS], color *colorStart,
	...
	calculateDistances(distanceMap, player.xLoc, player.yLoc, T_OBSTRUCTS_PASSABILITY, 0, false, true, true);
...
void printHelpScreen() {
	...
	char helpText[BROGUE_HELP_LINE_COUNT][DCOLS*3] = {
		...
		"",
		"   a/e/r/t/d/c  ****apply/equip/remove/throw/drop/call an item",
...
void printDiscoveries(unsigned long category, short start, short count, unsigned short itemCharacter, short x, short y, cellDisplayBuffer dbuf[COLS][ROWS]) {
	color *theColor, goodColor, badColor;
	char buf[COLS];
	short i, x2, magic;
	itemTable *theTable = tableForItemCategory(category);
	itemTable *theInheritedTable = inheritedTableForItemCategory(category);
	...
	// Only color summoning staff white when all summoning staffs are identified, however
	// display staff character when at least one has been identified.
	if (category == SUMMONING_STAFF) {
		theColor = &white;
		for (i = STAFF_ZOMBIE; i < NUMBER_STAFF_KINDS; i++) {
			if (staffTable[i].identified) {
				theTable[0].identified = true;
			} else {
				theColor = &darkGray;
			}
		}
		for (i = WAND_ZOMBIE; i < NUMBER_WAND_KINDS; i++) {
			if (wandTable[i].identified) {
				theTable[0].identified = true;
			} else {
				theColor = &darkGray;
			}
	...
	for (i = start; i < start + count; i++) {
		if (theTable[i].identified) {
			if (category != SUMMONING_STAFF) theColor = &white;
			plotCharToBuffer(itemCharacter, x, y + i, &itemColor, &black, dbuf);
		} else if (category == STAFF && wandTable[i].identified) {
			if (category != SUMMONING_STAFF) theColor = &white;
			plotCharToBuffer(WAND_CHAR, x, y + i, &itemColor, &black, dbuf);
		} else {
			theColor = &darkGray;
		}
		strcpy(buf, theTable[i].name);
		if (buf[0] == '\0' && theInheritedTable) {
			strcpy(buf, theInheritedTable[i].name);
		}
		upperCase(buf);
		strcat(buf, " ");
...
void printDiscoveriesScreen() {
	...
	printString("-- SCROLLS --", mapToWindowX(3), y = mapToWindowY(0), &flavorTextColor, &black, dbuf);
	printDiscoveries(SCROLL, 0, NUMBER_SCROLL_KINDS_SHOWN, SCROLL_CHAR, mapToWindowX(3), ++y, dbuf);

	printString("-- RINGS --", mapToWindowX(3), y += NUMBER_SCROLL_KINDS_SHOWN + 1, &flavorTextColor, &black, dbuf);
	printDiscoveries(RING, 0, NUMBER_RING_KINDS_SHOWN, RING_CHAR, mapToWindowX(3), ++y, dbuf);
	
	printString("-- POTIONS --", mapToWindowX(29), y = mapToWindowY(0), &flavorTextColor, &black, dbuf);
	printDiscoveries(POTION, 0, NUMBER_POTION_KINDS_SHOWN, POTION_CHAR, mapToWindowX(29), ++y, dbuf);// Skip stench, explosions and only display them if they are identified.
	if (potionTable[POTION_STENCH].identified) {
		printDiscoveries(POTION, POTION_STENCH, 1, POTION_CHAR, mapToWindowX(29), y++, dbuf); // Bump y up by one afterwards
	}
	if (potionTable[POTION_EXPLOSION].identified) {
		printDiscoveries(POTION, POTION_EXPLOSION, 1, POTION_CHAR, mapToWindowX(29), --y, dbuf); // Decrement by one before. This handles stench not being discovered.
	}
	
	y = mapToWindowY(0) + 1 + NUMBER_POTION_KINDS_SHOWN + (potionTable[POTION_STENCH].identified ? 1 : 0) + (potionTable[POTION_EXPLOSION].identified ? 1 : 0);
	
	printString("-- TALISMANS --", mapToWindowX(29), ++y, &flavorTextColor, &black, dbuf);
	printDiscoveries(TALISMAN, 0, NUMBER_TALISMAN_KINDS_SHOWN, TALISMAN_CHAR, mapToWindowX(29), ++y, dbuf);// Two talismans are pre-identified.
	
	printString("-- STAFFS OR WANDS --", mapToWindowX(54), y = mapToWindowY(0), &flavorTextColor, &black, dbuf);
	printDiscoveries(STAFF, 0, NUMBER_STAFF_KINDS_SHOWN, STAFF_CHAR, mapToWindowX(54), ++y, dbuf);// Skip all summoning staffs
	printDiscoveries(SUMMONING_STAFF, 0, 1, STAFF_CHAR, mapToWindowX(54), y + NUMBER_STAFF_KINDS_SHOWN, dbuf);//and replace with 'summoning'
...
short printMonsterInfo(creature *monst, short y, boolean dim, boolean highlight) {
	...
	if (!player.status[STATUS_HALLUCINATING] || rogue.playbackOmniscience || monst == &player) {
		...
		for (i=0; i<NUMBER_OF_STATUS_EFFECTS; i++) {
			...
			} else if (i == STATUS_STRONGER && monst->status[i] > 0) {
				sprintf(buf, "%s%i", statusStrings[STATUS_STRONGER], monst->strengthAmount);
				printProgressBar(0, y++, buf, monst->status[i], monst->maxStatus[i], &redBar, dim);
			...
			if (y < ROWS - 1) {
				...
				if (!rogue.armor || rogue.armor->flags & ITEM_IDENTIFIED) {
					...
					sprintf(buf, "Str: %s%i%s  Armor: %i",
							redColorEscape,
							rogue.strength + rogue.mightBonus + player.strengthAmount - player.weaknessAmount,
							grayColorEscape,
							displayedArmor);
				} else {
					sprintf(buf, "Str: %s%i%s  Armor: %i?",
...
unsigned long printCarriedItemDetails(item *theItem,
	...
	if (includeButtons) {
		...
		if (theItem->category & (FOOD | SCROLL | POTION | WAND | STAFF | CHARM)
			|| (theItem->category & (WEAPON) && theItem->kind == DART && rogue.talisman && rogue.talisman->kind == TALISMAN_ASSASSIN)
//			|| (theItem->category & TALISMAN && theItem == rogue.talisman && theItem->kind == TALISMAN_CHAOS)
			|| (theItem->category & TALISMAN && theItem == rogue.talisman && theItem->kind == TALISMAN_ALCHEMY)			 
			|| (theItem->category & TALISMAN && theItem == rogue.talisman && theItem->kind == TALISMAN_SHAPE_CHANGING)) {
			sprintf(buttons[b].text, "   %sa%spply   ", goldColorEscape, whiteColorEscape);
			buttons[b].hotkey[0] = APPLY_KEY;
			b++;
		}
		if (theItem->category & (ARMOR | WEAPON | SHIELD | TALISMAN | RING)) {
