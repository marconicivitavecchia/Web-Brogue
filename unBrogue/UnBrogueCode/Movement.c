...
void describedItemCategory(unsigned long theCategory, char *buf) {
	unsigned short itemCats[13] = {FOOD, WEAPON, ARMOR, POTION, ELIXIR, SCROLL, TOME, STAFF, WAND, RING, CHARM, TALISMAN, GOLD};
	
	assureCosmeticRNG;
	if (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience) {
		theCategory = itemCats[rand_range(0, 12)];
	}
	switch (theCategory) {
		case FOOD:
			strcpy(buf, "food");
...
void describeLocation(char *buf, short x, short y) {
	...
	if (monst
		...
		|| rogue.telepathyBonus >= distanceFastApproximation(player.xLoc, player.yLoc, monst->xLoc, monst->yLoc))) {
	...
	if (monst) {
		...
		if (theItem) {
			...
			// Potions float more easily than other items
			if (theItem->category & POTION && cellHasTerrainFlag(x, y, T_ALLOWS_SUBMERGING)) {
				subjectMoving = true;
			}
			...
			// Scrolls swirl in the wind
			} else if (theItem->category & SCROLL && cellHasTerrainFlag3(x, y, T3_IS_WIND)) {
				strcat(verb, " swirling");
				subjectMoving = true;
			} else {
...
void exposeCreatureToFire(creature *monst) {
	...
	if ((monst->status[STATUS_IMMUNE_TO_FIRE] && (monst != &player || !rogue.talisman || rogue.talisman->kind != TALISMAN_FLAMESPIRIT))
		|| (monst->bookkeepingFlags & MONST_SUBMERGED)
		|| ((!monst->status[STATUS_LEVITATING]) && cellHasTerrainFlag2(monst->xLoc, monst->yLoc, T2_EXTINGUISHES_FIRE))) {
		return;
	}
	if (monst->status[STATUS_BURNING] == 0) {
		if (monst == &player) {
			...
			if (rogue.talisman && rogue.talisman->kind == TALISMAN_FLAMESPIRIT) {
				updateRingBonuses();
...
boolean monsterShouldFall(creature *monst) {
	return (!(monst->status[STATUS_LEVITATING])
			...
			|| cellHasTerrainFlag2(monst->xLoc, monst->yLoc, T2_CAN_CLIMB))
...
void applyInstantTileEffectsToCreature(creature *monst) {
	...
	if (monst == &player
		&& !(monst->status[STATUS_LEVITATING])
		&& cellHasTerrainFlag(*x, *y, T_IS_SECRET)
		&& playerCanSee(*x, *y)) {
	...
	if (monst == &player) {
		if (rogue.inWater) {
			if (!cellHasTerrainFlag(player.xLoc, player.yLoc, T_IS_DEEP_WATER) || (player.status[STATUS_LEVITATING] || player.status[STATUS_BLESSED])
				|| cellHasTerrainFlag2(player.xLoc, player.yLoc, T2_CAN_CLIMB)
	...
	if (!(monst->status[STATUS_LEVITATING])
		...
		&& !(cellHasTerrainFlag(*x, *y, (T_OBSTRUCTS_PASSABILITY))
			 || cellHasTerrainFlag2(*x, *y, (T2_CAN_CLIMB)))
	...
	// Water puts out fire.
	if (cellHasTerrainFlag2(*x, *y, T2_EXTINGUISHES_FIRE)
	...
	if (cellHasTerrainFlag(*x, *y, T_PROMOTES_ON_PLAYER_ENTRY) && monst == &player) {
		// Subject to same caveats as T_PROMOTES_ON_STEP above.
		for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
			if (tileCatalog[pmap[*x][*y].layers[layer]].flags & T_PROMOTES_ON_PLAYER_ENTRY) {
				promoteTile(*x, *y, layer, false);
			}
		}
	}
	
	if (cellHasTerrainFlag2(*x, *y, T2_PROMOTES_WITH_ENTRY)) {
		// Subject to same caveats as T_PROMOTES_ON_STEP above.
		for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
			if (tileCatalog[pmap[*x][*y].layers[layer]].flags2 & T2_PROMOTES_WITH_ENTRY) {
				promoteTile(*x, *y, layer, false);
			}
		}
	}
	...
	if (cellHasTerrainFlag(*x, *y, T_ENTANGLES) && !monst->status[STATUS_STUCK]
		...
		&& (monst != &player || !rogue.talisman || rogue.talisman->kind != TALISMAN_SPIDER || rogue.talisman->enchant1 < 0)) {
	...
	if (cellHasTerrainFlag(*x, *y, T_CAUSES_EXPLOSIVE_DAMAGE) && !monst->status[STATUS_EXPLOSION_IMMUNITY]
		&& !(monst->bookkeepingFlags & MONST_SUBMERGED) && !(monst->info.flags & MONST_FIERY)) {
		...
		if (monst == &player) {
            ...
            if (rogue.armor && itemRunic(rogue.armor) && itemRune(rogue.armor) == A_DAMPENING) {
			...
			} else if (rogue.shield && itemRunic(rogue.shield) && itemRune(rogue.shield) == A_DAMPENING) {
                itemName(rogue.shield, buf2, false, false, NULL);
                sprintf(buf, "Your %s pulses and absorbs the damage.", buf2);
                messageWithColor(buf, &goodMessageColor, false);
	...
	// poisonous lichen
	if (cellHasTerrainFlag(*x, *y, T_CAUSES_POISON) && !(monst->info.flags & MONST_INANIMATE)
		&& !monst->status[STATUS_LEVITATING] && !monst->status[STATUS_POISON_IMMUNITY]) {
		if (monst == &player && !player.status[STATUS_POISONED]) {
			rogue.disturbed = true;
			if (rogue.talisman && rogue.talisman->kind == TALISMAN_SPIDER && !monst->status[STATUS_POISONED]) {
				updateRingBonuses();
			}
		}
	...
	if (cellHasTerrainFlag(*x, *y, T_CAUSES_CONFUSION) && !(monst->info.flags & MONST_INANIMATE)
		 && !monst->status[STATUS_CONFUSION_IMMUNITY]) {
		if (monst == &player) {
			rogue.disturbed = true;
		}
	...
	// paralysis gas
	if (cellHasTerrainFlag(*x, *y, T_CAUSES_PARALYSIS) && !(monst->info.flags & MONST_INANIMATE)
		&& !(monst->bookkeepingFlags & MONST_SUBMERGED) && !monst->status[STATUS_PARALYSIS_IMMUNITY]) {
...
void applyGradualTileEffectsToCreature(creature *monst, short ticks) {
	...
	if (!(monst->status[STATUS_LEVITATING] || monst->status[STATUS_BLESSED])
	...
	if (((cellHasTerrainFlag(x, y, T_CAUSES_DAMAGE) && !monst->status[STATUS_POISON_IMMUNITY])
		|| (cellHasTerrainFlag(x, y, T_CAUSES_FIERY_DAMAGE) && !(monst->info.flags & MONST_FIERY) && (monst->status[STATUS_IMMUNE_TO_FIRE_NOT_NEGATABLE] <= 0)))
		&& !(monst->info.flags & MONST_INANIMATE)
		&& !(monst->bookkeepingFlags & MONST_SUBMERGED)) {
		damage = (monst->info.maxHP / 15) * ticks / 100;
		damage = max(1, damage);
		for (layer = 0; layer < NUMBER_TERRAIN_LAYERS && !(tileCatalog[pmap[x][y].layers[layer]].flags & T_CAUSES_DAMAGE)
			 && !(tileCatalog[pmap[x][y].layers[layer]].flags & T_CAUSES_FIERY_DAMAGE); layer++);
		if (monst == &player) {
			rogue.disturbed = true;
	...
	if (cellHasTerrainFlag3(x, y, T3_CAUSES_HEALING)
		&& !(monst->info.flags & MONST_INANIMATE)
		&& !(monst->bookkeepingFlags & MONST_SUBMERGED)) {
...
boolean abortAttackAgainstAcidicTarget(creature *hitList[8], short hitCount) {
    short i;
    ...
    if (((rogue.weapon
		...
		((rogue.offhandWeapon) && !(rogue.offhandWeapon->flags & ITEM_PROTECTED)))
        ...
        for (i=0; i<hitCount; i++) {
            if (hitList[i]
                ...
                itemName(rogue.weapon && !(rogue.weapon->flags & ITEM_PROTECTED) ? rogue.weapon : rogue.offhandWeapon, weaponName, false, false, NULL);
...
{
	creature *defender;
	short newDir, newestX, newestY, i, hitCount = 0;
	
	defender = monsterAtLoc(newX, newY);
	if (defender) {
		hitList[hitCount++] = defender;
	}

	if ((attackFlags & ATTACKS_PENETRATE) && defender) {
	...
	} else if (attackFlags & ATTACKS_ALL_ADJACENT) {
		// Back up one grid
		newX = newX - nbDirs[direction][0];
		newY = newY - nbDirs[direction][1];
		for (i=0; i<8; i++) {
			newDir = (direction + i) % 8;
			newestX = newX + nbDirs[newDir][0];
			newestY = newY + nbDirs[newDir][1];
			if (coordinatesAreInMap(newestX, newestY) && (pmap[newestX][newestY].flags & HAS_MONSTER)) {
				defender = monsterAtLoc(newestX, newestY);
				if (hitCount && defender == hitList[0]) {
					continue;
				}
				if (defender
					&& monstersAreEnemies(&player, defender)
...
boolean playerMoves(short direction) {
	...
	short newX, newY, newestX, newestY, newImpaleDirection, hitCount = 0;
	boolean playerMoved = false, alreadyRecorded = false, dualWield = false;
	creature *defender = NULL, *hitList[8] = {NULL}, *tempMonst = NULL;
	char monstName[COLS];
	char buf[COLS];
...
//		if (!alreadyRecorded) { // PROBLEM: If the player does not vomit, and is moving into a pit, but then says no to the confirmation, then OOS.
...
//	}
	...
	{
		newestX = x+nbDirs[rogue.impaleDirection][0];
		newestY = y+nbDirs[rogue.impaleDirection][1];
		rogue.impaleDirection = NO_DIRECTION;
		refreshDungeonCell(newestX, newestY);
		if (!alreadyRecorded) {
			recordKeystroke(directionKeys[initialDirection], false, false);
			alreadyRecorded = true;
		}
	...
	// Set up impaling weapons before terrain promotions to prevent using spears/pikes through doors.
	if (rogue.weapon && (rogue.weapon->flags & ITEM_IMPALES) && !cellHasTerrainFlag(x, y, (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_VISION))) {
		newImpaleDirection = direction;
	} else {
		newImpaleDirection = NO_DIRECTION;
	}
    
    // If there's no enemy at the movement location that the player is aware of, consider terrain promotions.
    if (!(defender && (canSeeMonster(defender) || player.status[STATUS_TELEPATHIC] || rogue.telepathyBonus > 0) && monstersAreEnemies(&player, defender))) {
        
        if (!(~terrainFlags(newX, newY) & (T_PROMOTES_ON_PLAYER_ENTRY | T_OBSTRUCTS_PASSABILITY))) {
            layer = layerWithFlag(newX, newY, T_PROMOTES_ON_PLAYER_ENTRY);
	...
	if (((!cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY) || (cellHasTerrainFlag(newX, newY, T_PROMOTES_WITH_KEY) && keyInPackFor(newX, newY)))
		...
		if (defender) {
			// if there is a monster there, then we can't move
			newImpaleDirection = NO_DIRECTION;
			...
			if (defender->bookkeepingFlags & MONST_CAPTIVE) {
				...
				if (alreadyRecorded || confirm(buf, false)) {
					...
					if (cellHasTerrainFlag(newX, newY, T_PROMOTES_WITH_KEY) && keyInPackFor(newX, newY)) {
			...
			if (defender->creatureState != MONSTER_ALLY) {				
				hitCount = makeHitList(hitList, rogue.weapon ? rogue.weapon->flags : 0, newX, newY, direction);
				
				if (abortAttackAgainstAcidicTarget(hitList, hitCount)) { // Acid mound attack confirmation.
#ifdef BROGUE_ASSERTS
				...
				// Dual wielding players with a first weapon that is a dagger, sword or broadsword attack twice as quickly, as do those that
				// are wielding axes and axes, spears (or pikes) and spears (or pikes), and maces or warhammers and maces or warhammers
				if (rogue.weapon && rogue.offhandWeapon && (!(rogue.weapon->flags & (ITEM_ATTACKS_SLOWLY | ITEM_ATTACKS_QUICKLY | ITEM_ATTACKS_PENETRATE | ITEM_ATTACKS_ALL_ADJACENT))
															   || ((rogue.weapon->flags & (ITEM_ATTACKS_SLOWLY | ITEM_ATTACKS_QUICKLY | ITEM_ATTACKS_PENETRATE | ITEM_ATTACKS_ALL_ADJACENT))
															   == (rogue.offhandWeapon->flags & (ITEM_ATTACKS_SLOWLY | ITEM_ATTACKS_QUICKLY | ITEM_ATTACKS_PENETRATE | ITEM_ATTACKS_ALL_ADJACENT))))) {
					dualWield = true;
					if (rogue.weapon && (rogue.weapon->flags & ITEM_ATTACKS_SLOWLY)) {
						player.ticksUntilTurn += player.attackSpeed;
					} else if (rogue.weapon && (rogue.weapon->flags & ITEM_ATTACKS_QUICKLY)) {
						player.ticksUntilTurn += player.attackSpeed / 4;
					} else {
						player.ticksUntilTurn += player.attackSpeed / 2;
					}
				} else if (rogue.weapon && (rogue.weapon->flags & ITEM_ATTACKS_SLOWLY)) {
				...
				for (i=0; i<hitCount; i++) {
					if (hitList[i]
						...
						attack(&player, hitList[i], false, false);
						if (rogue.offhandWeaponNext) {
							rogue.offhandWeaponNext = false;
						} else {
							rogue.offhandWeaponNext = dualWield;
						}
					}
				}
		...
		if (player.bookkeepingFlags & MONST_SEIZED) {
			for (defender = monsters->nextCreature; defender != NULL; defender = defender->nextCreature) {
				if ((defender->bookkeepingFlags & MONST_SEIZING)
					&& monstersAreEnemies(&player, defender)
					&& distanceDiagonal(player.xLoc, player.yLoc, defender->xLoc, defender->yLoc) == 1
		...
		if (pmap[newX][newY].flags & (DISCOVERED | MAGIC_MAPPED)
				   ...
				   && !(cellHasTerrainFlag(newX, newY, T_IS_SECRET)
						|| cellHasTerrainFlag2(newX, newY, T2_CAN_CLIMB))) {
			...
			return false;
				   ...
				   && !cellHasTerrainFlag2(newX, newY, T2_EXTINGUISHES_FIRE)
        ...
        if (rogue.weapon && (rogue.weapon->flags & (ITEM_LUNGE_ATTACKS | ITEM_IMPALES))) {
            ...
            if (coordinatesAreInMap(newestX, newestY) && (pmap[newestX][newestY].flags & HAS_MONSTER)) {
                ...
                if (tempMonst
                    ...
                    hitCount = makeHitList(hitList, rogue.weapon->flags, newestX, newestY, direction);
                    if (abortAttackAgainstAcidicTarget(hitList, hitCount)) { // Acid mound attack confirmation.
#ifdef BROGUE_ASSERTS
                        assert(!alreadyRecorded);
#endif
		...
		// We are potentially impaling something
		if (newImpaleDirection >= 0) {
			newestX = newX + nbDirs[newImpaleDirection][0];
            newestY = newY + nbDirs[newImpaleDirection][1];
            if (coordinatesAreInMap(newestX, newestY) && !cellHasTerrainFlag(newestX, newestY, (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_VISION))) {
				if (pmap[newestX][newestY].flags & HAS_MONSTER) {
					tempMonst = NULL;
				} else {
					tempMonst = monsterAtLoc(newestX, newestY);
				}
                if (!tempMonst || (tempMonst->bookkeepingFlags & (MONST_SUBMERGED))) {
					if (!alreadyRecorded) {
						recordKeystroke(directionKeys[initialDirection], false, false);
						alreadyRecorded = true;
					}
					rogue.impaleDirection = newImpaleDirection;
					refreshDungeonCell(newestX, newestY);
				}
			}
		}
		...
		if (rogue.downLoc[0] == newX && rogue.downLoc[1] == newY) {
			...
			for (i=0; i<hitCount; i++) {
				if (hitList[i]
					&& monstersAreEnemies(&player, hitList[i])
					&& !(hitList[i]->bookkeepingFlags & MONST_IS_DYING)) {
					attack(&player, hitList[i], (rogue.weapon && (rogue.weapon->flags & (ITEM_LUNGE_ATTACKS))) != 0, false); // for the moment, always use the primary weapon
				}
			}
			moveEntrancedMonsters(direction);
...
void travel(short x, short y, boolean autoConfirm) {
	...
	calculateDistances(distanceMap, x, y, 0, &player, false, false, false);
	...
	for (i=0; i<DCOLS; i++) {
		for (j=0; j<DROWS; j++) {
            if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_PASSABILITY)
                && (!cellHasTerrainFlag(i, j, T_IS_SECRET) || (discoveredTerrainFlagsAtLoc(i, j) & T_OBSTRUCTS_PASSABILITY))) {
...
void populateCreatureCostMap(short **costMap, creature *monst) {
	...
	for (i=0; i<DCOLS; i++) {
		for (j=0; j<DROWS; j++) {
            ...
            if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_PASSABILITY)
				&& (!cellHasTerrainFlag(i, j, T_IS_SECRET) || (discoveredTerrainFlagsAtLoc(i, j) & T_OBSTRUCTS_PASSABILITY) || monst == &player)) {
				costMap[i][j] = cellHasTerrainFlag(i, j, T_OBSTRUCTS_DIAGONAL_MOVEMENT) ? PDS_OBSTRUCTION : PDS_FORBIDDEN;
                continue;
            }
            ...
            if (cellHasTerrainFlag(i, j, T_LAVA_INSTA_DEATH)
                ...
                && max(monst->status[STATUS_LEVITATING], monst->status[STATUS_IMMUNE_TO_FIRE]) < (rogue.mapToShore[i][j] + distanceDiagonal(i, j, monst->xLoc, monst->yLoc) * monst->movementSpeed / 100)) {
            ...
            if ((cellHasTerrainFlag(i, j, T_AUTO_DESCENT) || cellHasTerrainFlag(i, j, T_IS_DEEP_WATER) && !(monst->info.flags & MONST_IMMUNE_TO_WATER))
                ...
                && (monst->status[STATUS_LEVITATING])
                && monst->status[STATUS_LEVITATING] < (rogue.mapToShore[i][j] + distanceDiagonal(i, j, monst->xLoc, monst->yLoc) * monst->movementSpeed / 100)) {
            ...
            if (cellHasTerrainFlag(i, j, (T_CAUSES_NAUSEA)) || cellHasTerrainFlag2(i, j, T2_PROMOTES_ON_ITEM_PICKUP) || cellHasTerrainFlag3(i, j, T3_AVOID)
...
void updateTelepathy() {
	...
	for (i=0; i<DCOLS; i++) {
		for (j=0; j<DROWS; j++) {
			...
			if (pmap[i][j].flags & MARKED_VISIBLE) {
				pmap[i][j].flags |= WAS_TELEPATHIC_VISIBLE;
			}
			pmap[i][j].flags &= ~(TELEPATHIC_VISIBLE | MARKED_VISIBLE);
		}
	}
	
	if (player.status[STATUS_TELEPATHIC] || rogue.telepathyBonus > 0) {
		zeroOutGrid(grid);
		for (monst = monsters->nextCreature; monst; monst = monst->nextCreature) {
			if ((player.status[STATUS_TELEPATHIC] || rogue.telepathyBonus >= distanceFastApproximation(player.xLoc, player.yLoc, monst->xLoc, monst->yLoc))
			 && !(monst->info.flags & MONST_INANIMATE)) {
	...
	if (rogue.markedMonsters) {
		for (monst = monsters->nextCreature; monst; monst = monst->nextCreature) {
			if (monst->status[STATUS_MARKED]) {
				...
				getFOVMask(grid, monst->xLoc, monst->yLoc, 1.5, T_OBSTRUCTS_VISION, 0, false);
				pmap[monst->xLoc][monst->yLoc].flags |= (MARKED_VISIBLE | TELEPATHIC_VISIBLE | DISCOVERED);
			}
		}
		if (marked) {
			for (i = 0; i < DCOLS; i++) {
				for (j = 0; j < DROWS; j++) {
					if (grid[i][j]) {
						pmap[i][j].flags |= (MARKED_VISIBLE | TELEPATHIC_VISIBLE | DISCOVERED);
					}
				}
			}
		}
...
void updateVision(boolean refreshDisplay) {
	...
	if (player.status[STATUS_TELEPATHIC] || rogue.telepathyBonus > 0 || rogue.markedMonsters) {
		updateTelepathy();
	}
...
//	displayMoreSign();
	...
	for (theItem = floorItems->nextItem; theItem != NULL; theItem = theItem->nextItem) {
		if ((theItem->category & (POTION | SCROLL)) && (theItem->flags & ITEM_OVERCHARGED)
			&& theItem->xLoc >= 0 && theItem->yLoc >= 0 && theItem->xLoc < DCOLS && theItem->yLoc < DROWS // paranoia
			&& (pmap[theItem->xLoc][theItem->yLoc].flags & (DISCOVERED | ITEM_DETECTED))) {
			if (rogue.talisman && rogue.talisman->kind == TALISMAN_WITCHCRAFT) {
				theItem->flags &= ITEM_MAGIC_DETECTED;
				makeItemInto(theItem, (theItem->category & POTION ? ELIXIR : TOME), theItem->kind);
				if (refreshDisplay) {
					refreshDungeonCell(theItem->xLoc, theItem->yLoc);
				}
//			} else if (rogue.talisman && rogue.talisman->kind == TALISMAN_ALCHEMY && (theItem->category & POTION) && itemMagicChar(theItem) == BAD_MAGIC_CHAR) {
//				theItem->flags |= ITEM_MAGIC_DETECTED;
//				theItem->quantity = 2;
//				if (refreshDisplay) {
//					refreshDungeonCell(theItem->xLoc, theItem->yLoc);
...
void burnItem(item *theItem) {
	...
	boolean newBlessing = false;
	itemName(theItem, buf1, false, true, NULL);
	sprintf(buf2, "%s burns up!", buf1);

	if (rogue.talisman && rogue.talisman->kind == TALISMAN_SACRIFICE) {
		talismanTable[TALISMAN_SACRIFICE].identified = true;
		if (!player.status[STATUS_BLESSED]) {
			newBlessing = true;
		}
		player.status[STATUS_BLESSED] += itemValue(theItem) / 4;
		player.maxStatus[STATUS_BLESSED] = max(player.status[STATUS_BLESSED], player.maxStatus[STATUS_BLESSED]);
		refreshSideBar(-1, -1, false);
	}
	...
	if (newBlessing) {
		messageWithColor("the gods bless you for the sacrifice.", &goodMessageColor, false);
	}
}

boolean circuitBroken(short machineNumber) {
	short x, y;
	for (x=0; x<DCOLS; x++) {
		for (y=0; y<DROWS; y++) {
			if ((pmap[x][y].flags & IS_IN_MACHINE)
				...
				&& (cellHasTerrainFlag2(x, y, T2_CIRCUIT_BREAKER))) {
				return false;
			}
		}
	}
...
void activateMachine(short machineNumber, boolean circuitBroken) {
	...
	for (i=0; i<DCOLS; i++) {
		for (j=0; j<DROWS; j++) {
			...
			if ((pmap[x][y].flags & IS_IN_MACHINE)
				...
				|| (cellHasTerrainFlag2(x, y, T2_PROMOTES_WITH_CIRCUIT_BROKEN) && circuitBroken))) {
				...
				for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
					...
					if (circuitBroken && (tileCatalog[pmap[x][y].layers[layer]].flags2 & T2_PROMOTES_WITH_CIRCUIT_BROKEN)) {
						promoteTile(x, y, layer, false);
					}
				}
			}
	...
	activateMonstersinMachine(machineNumber, false);
}

void activateMonstersinMachine(short machineNumber, boolean called) {
		short i, monsterCount, maxMonsters;
		creature **activatedMonsterList, *monst;
		
    monsterCount = maxMonsters = 0;
    activatedMonsterList = NULL;
    for (monst = monsters->nextCreature; monst != NULL; monst = monst->nextCreature) {
		if (called && monst->info.abilityFlags & MA_CALL_GUARDIAN) {
			continue; // hack to prevent infinite recursion if we define a monster with both these flags 
		}
        if (monst->machineHome == machineNumber
			...
			i = rand_range(0, monsterCount-1);
			if (i < monsterCount-1) {
				activatedMonsterList[monsterCount - 1] = activatedMonsterList[i];
			}
            activatedMonsterList[i] = monst;
        }
    }
...
void promoteTile(short x, short y, enum dungeonLayers layer, boolean useFireDF) {
	...
	if ((!useFireDF || (tile->flags2 & T2_PROMOTES_WITH_FIRE)) && (tile->flags & T_IS_WIRED) && !(pmap[x][y].flags & IS_POWERED)) {
		...
		// If not a circuit breaker, or the last circuit breaker in the machine, we activate.
		if (!(tile->flags2 & T2_CIRCUIT_BREAKER) || circuitBroken(pmap[x][y].machineNumber)) {
			activateMachine(pmap[x][y].machineNumber, (tile->flags2 & T2_CIRCUIT_BREAKER) != 0); // It lives!!!
		
			// We need to double promote the last tile if the tile completes the circuit
			if ((tile->flags2 & T2_CIRCUIT_BREAKER) && (tileCatalog[dungeonFeatureCatalog[DFType].tile].flags2 & T2_PROMOTES_WITH_CIRCUIT_BROKEN)) {
				DFType = tileCatalog[dungeonFeatureCatalog[DFType].tile].promoteType;
			}
		}
...
boolean exposeTileToFire(short x, short y, boolean alwaysIgnite) {
	...
	for (layer=0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
		if ((tileCatalog[pmap[x][y].layers[layer]].flags2 & T2_EXTINGUISHES_FIRE)
	...
	for (i=0; i<DCOLS; i++) {
		for (j=0; j<DROWS; j++) {
			if (!cellHasTerrainFlag(i, j, T_OBSTRUCTS_GAS)) {
				...
				if (cellHasTerrainFlag3(i, j, T3_IS_WIND) && !(tileCatalog[gasType].flags3 & T3_IS_WIND)) { // wind disperses gases a lot faster except wind
					numSpaces++;
				}
...
void updateEnvironment() {
	...
	for (i=0; i<DCOLS; i++) {
		for (j=0; j<DROWS; j++) {
			...
			if (cellHasTerrainFlag2(i, j, T2_PROMOTES_WITHOUT_ENTRY) && !(pmap[i][j].flags & (HAS_PLAYER | HAS_MONSTER))) {
				for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
					if (tileCatalog[pmap[i][j].layers[layer]].flags2 & T2_PROMOTES_WITHOUT_ENTRY) {
						promoteTile(i, j, layer, false);
					}
				}
			}
			if (cellHasTerrainFlag2(i, j, T2_IS_SPARKING)) {
				loc[0] = i;
				loc[1] = j;
				spark(loc, 2, 1);
			}
...
void updateAllySafetyMap() {
	...
	playerCostMap = allocDynamicGrid();
	monsterCostMap = allocDynamicGrid();
		...
		for (j=0; j<DROWS; j++) {
			...
			if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_PASSABILITY)
				...
				if (cellHasTerrainFlag(i, j, (T_AUTO_DESCENT | T_IS_DF_TRAP))) {// | T_IS_FIRE))) {
                    ...
                    if (player.status[STATUS_LEVITATING]) {
                        playerCostMap[i][j] = 1;
                    } else if (player.status[STATUS_BLESSED] && cellHasTerrainFlag(i, j, (T_IS_DEEP_WATER))) {
                        playerCostMap[i][j] = 1;
                    } else {
                        playerCostMap[i][j] = 5;
                    }
	...
	for (i=0; i<DCOLS; i++) {
		for (j=0; j<DROWS; j++) {
			...
			if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_PASSABILITY)
				...
				|| (cellHasTerrainFlag(i, j, T_PATHING_BLOCKER & ~T_HARMFUL_TERRAIN) && !cellHasTerrainFlag(i, j, T_IS_SECRET))) {
				costMap[i][j] = PDS_FORBIDDEN;
...
void rechargeItemsIncrementally() {
	item *theItem, *autoIdentifyItems[5] = {rogue.armor, rogue.shield, rogue.ringLeft, rogue.ringRight, rogue.ringThird};
	char buf[DCOLS], theItemName[DCOLS];
	short rechargeIncrement, i;
	unsigned long staffHasFlags = 0;
		...
		if (theItem->category & STAFF) {
			if (theItem->enchant2 <= 0) {
				...
				for (i = rogue.talisman && rogue.talisman->kind == TALISMAN_WIZARDRY ? 0 : 1; i < theItem->numberOfBoltEnchants; i++) {
					staffHasFlags |= boltRunicCatalog[theItem->boltEnchants[i]].flags;
				}

				// staffs of blinking and obstruction recharge half as fast so they're less powerful
				theItem->enchant2 = (theItem->kind == STAFF_BLINKING || theItem->kind == STAFF_OBSTRUCTION ? (staffHasFlags & BOLT_EASY_CHARGING ? 5000 : 10000) : 
									 (staffHasFlags & BOLT_EASY_CHARGING ? 2500 : 5000)) / theItem->enchant1;
...
void decrementPlayerStatus() {
	...
	if (rogue.talisman && rogue.talisman->kind == TALISMAN_MADNESS && player.status[STATUS_HALLUCINATING]) {
		rogue.talismanFraction += (int) (100 / madnessBoost(rogue.talisman->enchant1));
		player.status[STATUS_HALLUCINATING] += (int) 1 - rogue.talismanFraction / 100;
		rogue.talismanFraction %= 100;
		if (player.status[STATUS_HALLUCINATING] <= 0) player.status[STATUS_HALLUCINATING] = 1;
		talismanTable[TALISMAN_MADNESS].identified = true;
		rogue.talisman->flags |= ITEM_IDENTIFIED;
		rogue.talisman->flags &= ~ITEM_CAN_BE_IDENTIFIED;
	}
	
	if (rogue.talisman && rogue.talisman->kind == TALISMAN_SPIDER && player.status[STATUS_POISONED]) {
		rogue.talismanFraction += (int) (100 / poisonBoost(rogue.talisman->enchant1));
		player.status[STATUS_POISONED] += (int) 1 - rogue.talismanFraction / 100;
		rogue.talismanFraction %= 100;
		if (player.status[STATUS_POISONED] <= 0) player.status[STATUS_POISONED] = 1;
		talismanTable[TALISMAN_SPIDER].identified = true;
		rogue.talisman->flags |= ITEM_IDENTIFIED;
		rogue.talisman->flags &= ~ITEM_CAN_BE_IDENTIFIED;
	}
	
	if (rogue.talisman && rogue.talisman->kind == TALISMAN_FLAMESPIRIT && player.status[STATUS_BURNING]) {
		rogue.talismanFraction += (int) (100 / fireDancingBoost(rogue.talisman->enchant1));
		player.status[STATUS_BURNING] += (int) 1 - rogue.talismanFraction / 100;
		rogue.talismanFraction %= 100;
		if (player.status[STATUS_BURNING] <= 0) player.status[STATUS_BURNING] = 1;
		talismanTable[TALISMAN_FLAMESPIRIT].identified = true;
		rogue.talisman->flags |= ITEM_IDENTIFIED;
		rogue.talisman->flags &= ~ITEM_CAN_BE_IDENTIFIED;
	}
	...
	if (player.status[STATUS_LIGHTENED] > 0) {
		player.status[STATUS_LIGHTENED]--;
		updateMinersLightRadius();
		//updateVision();
	}
	...
	if (player.status[STATUS_LEVITATING_NOT_NEGATABLE] > 0 && !--player.status[STATUS_LEVITATING_NOT_NEGATABLE]) {
	...
	if (player.status[STATUS_HASTED_NOT_NEGATABLE] > 0 && !--player.status[STATUS_HASTED_NOT_NEGATABLE]) {
		player.movementSpeed = player.info.movementSpeed / 2; // slow down to merely hasted
		player.attackSpeed = player.info.attackSpeed / 2;
		if (!player.status[STATUS_HASTED]) { // should never happen
			player.status[STATUS_HASTED] = 1;
		}
	}	
	...
	if (player.status[STATUS_STRONGER] > 0 && !--player.status[STATUS_STRONGER]) {
		player.strengthAmount = 0;
		message("your muscles feel lifeless and weak.", false);
		updateEncumbrance();
	}
	
	if (player.status[STATUS_IMMUNE_TO_FIRE_NOT_NEGATABLE] > 0 && !--player.status[STATUS_IMMUNE_TO_FIRE_NOT_NEGATABLE]) {
	...
	if (player.status[STATUS_POISON_IMMUNITY] > 0 && !--player.status[STATUS_POISON_IMMUNITY]) {
		message("you no longer feel immune to poison.", false);
	}
	
	if (player.status[STATUS_CONFUSION_IMMUNITY] > 0 && !--player.status[STATUS_CONFUSION_IMMUNITY]) {
		message("you no longer feel immune to confusion.", false);
	}
	
	if (player.status[STATUS_PARALYSIS_IMMUNITY] > 0 && !--player.status[STATUS_PARALYSIS_IMMUNITY]) {
		message("you no longer feel immune to paralysis.", false);
	}
	...
	if (player.status[STATUS_SHIELDED]) {
		player.protectionAmount -= player.protectionAmount / player.status[STATUS_SHIELDED];
		player.status[STATUS_SHIELDED]--;
	}
    
	if (player.status[STATUS_IMPROVED_INVISIBLE] > 0 && !--player.status[STATUS_IMPROVED_INVISIBLE]) {
		// nothing
	}
	...
	if (player.status[STATUS_REFLECTIVE] > 0 && !--player.status[STATUS_REFLECTIVE]) {
		message("you are no longer reflective.", false);
	}
	
	if (player.status[STATUS_BLESSED] > 0 && !--player.status[STATUS_BLESSED]) {
		message("you are no longer blessed.", false);
	}
	
	if (player.status[STATUS_LIFESPAN_REMAINING] > 0 && !--player.status[STATUS_LIFESPAN_REMAINING]) {
		inflictLethalDamage(&player);
	}
...
void handleXPXP() {
	...
	// rough break down as follows: turns for full regen normally is 300. a level is (79 * 29) = 2290 grids of which around
	// 800 reward experience. so if we treat grids = turns, we need 1 hp per 3 grids for a starting character with 30 hp if we rest once fully on a level
	// note that we passively rest while exploring a level, so we should actually reduce this to 1 hp per 2 grids or even 1 hp per grid
	// the rogue hit point total then scales up from 30 hp to 110 hp so need a scale factor of 1 to ~3.6 across the dungeon, which 26/7 is a good approximation.	
	if (rogue.talisman && rogue.talisman->kind == TALISMAN_DUNGEONEERING) {
		xpxp = rogue.xpxpThisTurn * clamp(rogue.depthLevel, 7, 26) / 7 + player.xpxp;
		for (; xpxp > 2 && player.currentHP < player.info.maxHP; xpxp -= 2) {
			player.currentHP++;
			if (rogue.dungeoneeringDebt < 30000) {
				rogue.dungeoneeringDebt++;
			}
		}
		if (player.currentHP == player.info.maxHP) {
			player.protectionAmount += xpxp * 5; // 10/grids per hp
			player.status[STATUS_SHIELDED] = min(20, player.status[STATUS_SHIELDED] + xpxp);
			player.maxStatus[STATUS_SHIELDED] = min(20, player.maxStatus[STATUS_SHIELDED] + xpxp);
			player.xpxp = 0;
		} else {
			player.xpxp = xpxp;
		}
	} else if (rogue.dungeoneeringDebt) {
		xpxp += player.xpxp;
		if (rogue.dungeoneeringDebt < xpxp / 2) {
			rogue.dungeoneeringDebt = 0;
		} else {
			rogue.dungeoneeringDebt -= xpxp / 2;
		}
		player.xpxp = 0;
	}
...
void playerFalls() {
	...
	if (cellHasTerrainFlag(player.xLoc, player.yLoc, T_IS_SECRET)
	...
	if (rogue.depthLevel < 100) {
		...
		if (cellHasTerrainFlag(player.xLoc, player.yLoc, T_ALLOWS_SUBMERGING)
			|| cellHasTerrainFlag(player.xLoc, player.yLoc, T_PROMOTES_ON_STEP)
			|| cellHasTerrainFlag3(player.xLoc, player.yLoc, T3_IS_WIND)) {
			damage /= 2;
			
			layer = layerWithFlag(player.xLoc, player.yLoc, T_PROMOTES_ON_STEP);
			if (layer == -1) {
				layer = layerWithFlag(player.xLoc, player.yLoc, T_ALLOWS_SUBMERGING);
			}
			
			if (layer == -1) {
				messageWithColor("A gust of wind catches you.", &goodMessageColor, false);
			} else {
				sprintf(buf, "%s breaks your fall.", tileCatalog[pmap[player.xLoc][player.yLoc].layers[layer]].description);
				messageWithColor(buf, &goodMessageColor, false);
			}
		} else {
...
void playerTurnEnded() {
	...
	if (player.bookkeepingFlags & MONST_IS_FALLING) {
		monstersFall();	// do this first so they fall with the player rather than getting suspended
	...
	do {
		...
		} else if (rogue.dungeoneeringDebt) { // don't naturally regnerate, but allow rings of regeneration
		...
		while (player.ticksUntilTurn > 0) {
			...
			if (rogue.ticksTillUpdateEnvironment <= 0) {
				...
				rechargeItemsIncrementally(); // staffs and charms recharge every so often
		...
		for(monst = monsters->nextCreature; monst != NULL; monst = monst->nextCreature) {
			if (canSeeMonster(monst) && !(monst->bookkeepingFlags & (MONST_WAS_VISIBLE))) {
				...
				if (monst->creatureState != MONSTER_ALLY) {
					...
					if (rogue.cautiousMode || rogue.automationActive) {
						...
						sprintf(buf, "you %s a%s %s",
								(pmap[monst->xLoc][monst->yLoc].flags & VISIBLE) ? "see" : "sense",
								(isVowel(buf2) ? "n" : ""),
								buf2);
				...
				if (rogue.weapon && itemRunic(rogue.weapon) && itemRune(rogue.weapon) == W_SLAYING
				...
				if (rogue.armor && itemRunic(rogue.armor) && itemRune(rogue.armor) == A_IMMUNITY
				...
				if (rogue.shield && itemRunic(rogue.shield) && itemRune(rogue.shield) == A_IMMUNITY
					&& !(rogue.shield->flags & ITEM_RUNIC_HINTED)
					&& rogue.shield->vorpalEnemy == monst->info.monsterID) {
	...
	if ((player.status[STATUS_LEVITATING] && cellHasTerrainFlag(player.xLoc, player.yLoc, T_LAVA_INSTA_DEATH | T_IS_DEEP_WATER | T_AUTO_DESCENT))
		|| (player.status[STATUS_BLESSED] && cellHasTerrainFlag(player.xLoc, player.yLoc, T_IS_DEEP_WATER))
		...
		if (!rogue.receivedLevitationWarning) {
			...
			} else if (cellHasTerrainFlag(player.xLoc, player.yLoc, T_IS_DEEP_WATER)) {
				turnsToShore = max(player.status[STATUS_LEVITATING], player.status[STATUS_BLESSED]) * 100 / player.movementSpeed;
			} else {
...
void resetScentTurnNumber() { // don't want player.scentTurnNumber to roll over the unsigned short maxint!
	short i, j;
...
boolean isDisturbed(short x, short y) {
	...
	for (i=0; i<8; i++) {
		...
		if (monst
			&& !(monst->creatureState == MONSTER_ALLY)
			&& (canSeeMonster(monst) || player.status[STATUS_TELEPATHIC] || rogue.telepathyBonus > 0)) {
	...
	for (i = x - radius; i <= x + radius; i++) {
		for (j = y - radius; j <= y + radius; j++) {
			if (coordinatesAreInMap(i, j)
                ...
                percent = searchStrength - distanceDiagonal(x, y, i, j) * 10;
                if (cellHasTerrainFlag(i, j, T_OBSTRUCTS_PASSABILITY)) {
                    percent *= .667;
                }
...
boolean useStairs(short stairDirection) {
	...
	if (stairDirection == 1) {
        if (rogue.depthLevel == 100) {
            message("You are already at the bottom of the universe.", false);
            return false;
        }
		...
		if (rogue.depthLevel > 1 || numberOfMatchingPackItems(AMULET, 0, 0, false)) {
			...
			confirmMessages();
			message("the dungeon exit is magically sealed!", false);
		}
	}
						   ...
						   if (refreshDisplay) {
							   refreshDungeonCell(i, j);
								  ...
								  && ((tileCatalog[pmap[i][j].layers[DUNGEON]].backColor) && tileCatalog[pmap[i][j].layers[DUNGEON]].backColor->colorDances
									  ...
									  || (player.status[STATUS_HALLUCINATING] && (pmap[i][j].flags & ANY_KIND_OF_VISIBLE)))) {
