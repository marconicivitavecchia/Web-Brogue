...
void calculateDistances(short **distanceMap,
	...
	for (i=0; i<DCOLS; i++) {
		for (j=0; j<DROWS; j++) {
			char cost;
			if (canPenetrateStatues && tileCatalog[pmap[i][j].layers[DUNGEON]].displayChar == STATUE_CHAR) {
				cost = 1;
			} else if (canUseSecretDoors && pmap[i][j].layers[DUNGEON] == SECRET_DOOR) {
				cost = 1;
...
short pathingDistance(short x1, short y1, short x2, short y2, unsigned long blockingTerrainFlags) {
	...
	calculateDistances(distanceMap, x2, y2, blockingTerrainFlags, NULL, false, true, true);
