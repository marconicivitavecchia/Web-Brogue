...
boolean monsterIsHidden(creature *monst) {
	return ((monst->status[STATUS_INVISIBLE] && monst->creatureState != MONSTER_ALLY && !pmap[monst->xLoc][monst->yLoc].layers[GAS]) // invisible and not ally
			...
			|| (monst->bookkeepingFlags & MONST_IS_DORMANT)				// or monster is dormant
			|| (rogue.telepathyBonus < 0 && distanceFastApproximation(player.xLoc, player.yLoc, monst->xLoc, monst->yLoc) + rogue.telepathyBonus <= 0)) // or wearing cursed telepathy ring
			? true : false;				// or monster is dormant
}
...
boolean monstersAreEnemies(creature *monst1, creature *monst2) {
	...
	// nagas and salamanders always fight
	if ((monst1->info.monsterID == MK_NAGA && monst2->info.monsterID == MK_SALAMANDER) || 
		(monst1->info.monsterID == MK_SALAMANDER && monst2->info.monsterID == MK_NAGA)) {
		return true;
	}
	// eels and krakens attack anything in deep water; and anything at all if they are in a machine
	if (((monst1->info.flags & MONST_RESTRICTED_TO_LIQUID)
		 && !(monst2->info.flags & MONST_IMMUNE_TO_WATER)
		 && !(monst2->status[STATUS_LEVITATING])
		 && ((monst1->bookkeepingFlags & MONST_ATTACK_SHALLOWS) || cellHasTerrainFlag(monst2->xLoc, monst2->yLoc, T_IS_DEEP_WATER)))
		
		|| ((monst2->info.flags & MONST_RESTRICTED_TO_LIQUID)
			&& !(monst1->info.flags & MONST_IMMUNE_TO_WATER)
			&& !(monst1->status[STATUS_LEVITATING])
			&& ((monst2->bookkeepingFlags & MONST_ATTACK_SHALLOWS) || cellHasTerrainFlag(monst1->xLoc, monst1->yLoc, T_IS_DEEP_WATER)))) {
...
creature *cloneMonster(creature *monst, boolean announce, boolean placeClone) {
	...
	*newMonst = *monst; // boink!
	...
	newMonst->bookkeepingFlags |= (MONST_FOLLOWER | MONST_NO_EXPERIENCE);
...
boolean spawnMinions(short hordeID, creature *leader, boolean summoned) {
	...
	unsigned long forbiddenTerrainFlags, forbiddenMapFlags;
	hordeType *theHorde;
	creature *monst;
	short x, y, candidateCount;
	short failsafe;
	...
	for (iSpecies = 0; iSpecies < theHorde->numberOfMemberTypes; iSpecies++) {
		...
		forbiddenMapFlags = (HAS_PLAYER | HAS_MONSTER | IS_IN_MACHINE);
		forbiddenMapFlags &= ~(pmap[x][y].flags & IS_IN_MACHINE); // Allowed to spawn in an area with the same machine flags as the leader's location.
		
		for (iMember = 0; iMember < count; iMember++) {
			monst = generateMonster(theHorde->memberType[iSpecies], true);
			failsafe = 0;
			do {
				getQualifyingLocNear(loc, x, y, summoned, 0, forbiddenTerrainFlags, forbiddenMapFlags, false, false);
...
//	}
	...
	if (requiredFlags & HORDE_ASSASSINATION_TARGET) {
		leader->bookkeepingFlags |= (MONST_ASSASSINATION_TARGET | MONST_ASSASSINATION_ALERT);
	}
	if ((requiredFlags & HORDE_ASSASSINATION_TARGET) || (hordeCatalog[hordeID].flags & HORDE_MARKED)) {
		leader->status[STATUS_MARKED] = 1000;
	...
	if (hordeCatalog[hordeID].flags & HORDE_LEADER_CAPTIVE) {
		...
		// ensure captives can't learn experience prior to their minimum depth
		if (hordeCatalog[hordeID].minLevel > rogue.depthLevel) {
			leader->spawnDepth = hordeCatalog[hordeID].minLevel;
		}
	...
	if (hordeCatalog[hordeID].flags & HORDE_DORMANT) {
		toggleMonsterDormancy(leader);
		if (leader->creatureState != MONSTER_ALLY) {
			leader->creatureState = MONSTER_TRACKING_SCENT;
		}
	}
...
boolean summonMinions(creature *summoner) {
	...
	for (monst = monsters->nextCreature; monst != NULL; monst = monst->nextCreature) {
		if (monst != summoner && monstersAreTeammates(monst, summoner)
			...
			monst->bookkeepingFlags |= MONST_NO_EXPERIENCE; // no farming summoners to learn about your weapons
...
void populateMonsters() {
	...
	for (i=0; i<numberOfMonsters; i++) {
		spawnHorde(0, -1, -1, (HORDE_IS_SUMMONED | HORDE_MACHINE_ONLY),
				   (!i && rogue.talisman && rogue.talisman->kind == TALISMAN_ASSASSIN ? HORDE_ASSASSINATION_TARGET : 0)); // random horde type, random location
	}
}
...
void spawnPeriodicHorde() {
	...
	} while ((pmap[loc[0]][loc[1]].flags & (IN_FIELD_OF_VIEW | ANY_KIND_OF_VISIBLE | IS_IN_MACHINE)) ||
			 distanceDiagonal(player.xLoc, player.yLoc, loc[0], loc[1]) < 15);
...
void teleport(creature *monst) {
	...
	do {
		randomMatchingLocation(&x, &y, FLOOR, NOTHING, -1);
	} while (((pmap[x][y].flags & IS_IN_MACHINE) || grid[x][y])
			 && --failsafe);
...
boolean monsterAvoids(creature *monst, short x, short y) {
	...
	if (cellHasTerrainFlag(x, y, T_OBSTRUCTS_PASSABILITY)) {
        ...
        if (distanceDiagonal(monst->xLoc, monst->yLoc, x, y) <= 1) {
	...
	if (!(monst->status[STATUS_LEVITATING])) { // monster doesn't fly
...
boolean moveMonsterPassivelyTowards(creature *monst, short targetLoc[2], boolean willingToAttackPlayer) {
	...
	if (monsterAvoids(monst, newX, newY) || (!willingToAttackPlayer && (pmap[newX][newY].flags & HAS_PLAYER)) || !moveMonster(monst, dx, dy)) {
		if (distanceDiagonal(x, y, targetLoc[0], targetLoc[1]) <= 1 && (dx == 0 || dy == 0)) { // cardinally adjacent
...
// For roguelikes like Brogue where sqrt(2) = 1.
short distanceDiagonal(short x1, short y1, short x2, short y2) {
	return max(abs(x1 - x2), abs(y1 - y2));
}

// For roguelikes like Cardinal quest where there are no diagonals
short distanceManhattan(short x1, short y1, short x2, short y2) {
	return abs(x1 - x2) + abs(y1 - y2);
}

// Returns the square of the distance. Use for fast comparisons.
...
short awarenessDistance(creature *observer, creature *target) {
	...
	if ((observer->status[STATUS_LEVITATING] || (observer->info.flags & MONST_RESTRICTED_TO_LIQUID) || (observer->bookkeepingFlags & MONST_SUBMERGED)
		...
		&& ((target == &player && (pmap[observer->xLoc][observer->yLoc].flags & IN_FIELD_OF_VIEW)) ||
			...
			perceivedDistance = distanceDiagonal(observer->xLoc, observer->yLoc, target->xLoc, target->yLoc) * 3;
		} else {
...
void updateMonsterState(creature *monst) {
    ...
    if (player.status[STATUS_INVISIBLE]) {
		...
		if (player.status[STATUS_IMPROVED_INVISIBLE]) {
            maximumInvisibilityDetectionRadius--;
		}
        CYCLE_MONSTERS_AND_PLAYERS(monst2) {
            if ((monst2 == &player || monstersAreEnemies(monst, monst2))
                ...
                && monst != monst2
                && distanceDiagonal(monst->xLoc, monst->yLoc, monst2->xLoc, monst2->yLoc) <= maximumInvisibilityDetectionRadius) {
...
//			}
		...
		chooseNewWanderDestination(monst);
			   ...
			   && ((monst->info.flags & MONST_MAINTAINS_DISTANCE)
				   || ((monst->info.abilityFlags & MA_POISONS) && (player.status[STATUS_POISONED] || player.status[STATUS_POISON_IMMUNITY])))
			   && distanceDiagonal(x, y, player.xLoc, player.yLoc) < 3
		...
		monst->creatureState = (((monst->bookkeepingFlags & MONST_FOLLOWER) && monst->leader == &player)
			   ...
			   && !player.status[STATUS_POISON_IMMUNITY]) {
		monst->creatureState = MONSTER_TRACKING_SCENT;
	}
}
...
void decrementMonsterStatus(creature *monst) {
	...
	if (monst->currentHP < monst->info.maxHP && monst->info.turnsBetweenRegen > 0 && !monst->status[STATUS_POISONED]) {
		...
		if (monst->bookkeepingFlags & MONST_SUBMERGED && !(monst->info.flags & MONST_RESTRICTED_TO_LIQUID)) {
			monst->currentHP += 2;
		}
	}
	
	for (i=0; i<NUMBER_OF_STATUS_EFFECTS; i++) {
		if (monst->status[i] == 1000) {
			continue;
		}
        switch (i) {
            ...
            case STATUS_SHIELDED:
				if (monst->status[i]) {
					monst->protectionAmount -= monst->protectionAmount / monst->status[i];
					monst->status[i]--;
				}
                break;
			...
			case STATUS_MARKED:
                ...
                break;
			case STATUS_AGGRAVATED:
                if (monst->status[i]
                    && !--monst->status[i]) {
					monst->info.flags |= (monst->aggravatedBehaviors & ~(monst->negatedBehaviors));
					monst->info.abilityFlags |= (monst->aggravatedAbilities & ~(monst->negatedAbilities));
					monst->aggravatedAbilities = 0L;
					monst->aggravatedBehaviors = 0L;
					// assume we don't cancel any abilities below due to aggravation
				}
                break;
			case STATUS_NEGATED:
                if (monst->status[i]
                    && !--monst->status[i]) {
					monst->info.flags |= (monst->negatedBehaviors & ~(monst->aggravatedBehaviors));
					monst->info.abilityFlags |= (monst->negatedAbilities & ~(monst->aggravatedAbilities));
					monst->negatedAbilities = 0L;
					monst->negatedBehaviors = 0L;
					if (monst->info.flags & MONST_FIERY) {
						monst->status[STATUS_BURNING] = monst->maxStatus[STATUS_BURNING] = 1000; // won't decrease
					}
					if (monst->info.flags & MONST_FLIES) {
                ...
                break;				
			case STATUS_POLYMORPHED:
                if (monst->status[i]
                    && !--monst->status[i]) {
					polymorph(monst, monst->polymorphedFrom, 0);
                }
                break;
			case STATUS_DOMINATED:
                if (monst->status[i]
                    && !--monst->status[i]) {
					unAlly(monst);
                }
...
void moveTowardLeader(creature *monst) {
	...
	if (!monst->leader->mapToMe) {
		...
		calculateDistances(monst->leader->mapToMe, monst->leader->xLoc, monst->leader->yLoc, 0, monst, false, true, false);
	}
	...
	if ((monst->info.abilityFlags & MA_CAST_BLINK)
		&& distanceDiagonal(monst->xLoc, monst->yLoc, monst->leader->xLoc, monst->leader->yLoc) > 10
...
boolean monsterBlinkToPreferenceMap(creature *monst, short **preferenceMap, boolean blinkUphill) {
	...
	maxDistance = boltBlinkDistance(5);
	gotOne = false;
	...
	if (gotOne) {
		...
		monst->ticksUntilTurn = monst->attackSpeed * (monst->info.flags & MONST_CAST_SPELLS_SLOWLY ? 2 : 1);
		zap(origin, bestTarget, BOLT_BLINKING, 5, 1.0, 0L);
		return true;
	}
	return false;
}
...
boolean monstUseMagic(creature *monst) {
	...
	if (!monst->status[STATUS_CONFUSED] && monst->info.abilityFlags
		& (MA_CAST_NEGATION | MA_CAST_SLOW | MA_CAST_DISCORD | MA_CAST_BECKONING | MA_BREATHES_FIRE | MA_SHOOTS_WEBS | MA_ATTACKS_FROM_DISTANCE | MA_CAST_SENTRY)) {
		...
		CYCLE_MONSTERS_AND_PLAYERS(target) {
			if (monstersAreEnemies(monst, target)
                ...
                // mirrored totems, lamias sometimes cast beckoning
				if ((monst->info.abilityFlags & MA_CAST_BECKONING)
					&& !(target->info.flags & MONST_INANIMATE)
                    && distanceDiagonal(monst->xLoc, monst->yLoc, targetLoc[0], targetLoc[1]) > 1
					&& !((monst->info.abilityFlags & MA_POISONS) && (player.status[STATUS_POISONED] || player.status[STATUS_POISON_IMMUNITY]))
                    ...
                    zap(originLoc, targetLoc, BOLT_BECKONING, 10, 1.0, 0L);
                    return true;
                }
				...
				if ((monst->info.abilityFlags & MA_BREATHES_FIRE)
                    ...
                    if (!(burnedTerrainFlagsAtLoc(listOfCoordinates[0][0], listOfCoordinates[0][1]) & T_IS_FIRE)
                        ...
                        zap(originLoc, targetLoc, BOLT_FIRE, 18, 1.0, 0L);
				...
				if (monst->info.abilityFlags & MA_SHOOTS_WEBS
					...
					&& (!target->status[STATUS_STUCK] || distanceDiagonal(monst->xLoc, monst->yLoc, targetLoc[0], targetLoc[1]) > 1)
					&& distanceDiagonal(monst->xLoc, monst->yLoc, targetLoc[0], targetLoc[1]) < 20
					&& ((!target->status[STATUS_STUCK] && distanceDiagonal(monst->xLoc, monst->yLoc, targetLoc[0], targetLoc[1]) == 1)
				...
				if ((monst->info.abilityFlags & MA_ATTACKS_FROM_DISTANCE)
					...
					&& distanceDiagonal(monst->xLoc, monst->yLoc, targetLoc[0], targetLoc[1]) < 11
				...
				if (monst->info.abilityFlags & (MA_CAST_DISCORD)
					...
					zap(originLoc, targetLoc, BOLT_DISCORD, 10, 1.0, 0L);
					return true;
				}
				
				// Opportunity to cast negation cleverly?
				if (monst->info.abilityFlags & (MA_CAST_NEGATION)
					&& ((target->status[STATUS_HASTED] && !target->status[STATUS_HASTED_NOT_NEGATABLE]) || target->status[STATUS_TELEPATHIC] || target->status[STATUS_SHIELDED]
						|| (target->info.flags & (MONST_DIES_IF_NEGATED | MONST_IMMUNE_TO_WEAPONS))
						|| (((target->status[STATUS_IMMUNE_TO_FIRE] && !target->status[STATUS_IMMUNE_TO_FIRE_NOT_NEGATABLE])
						|| (target->status[STATUS_LEVITATING] && !target->status[STATUS_LEVITATING_NOT_NEGATABLE]))
							&& cellHasTerrainFlag(target->xLoc, target->yLoc, (T_LAVA_INSTA_DEATH | T_IS_DEEP_WATER | T_AUTO_DESCENT))))
					&& !(target == &player && rogue.armor && (((rogue.armor->flags & ITEM_RUNIC) && rogue.armor->enchant2 == A_REFLECTION)
															  || (rogue.talisman && rogue.talisman->kind == TALISMAN_RUNE_MASTERY
																  && (rogue.armor->hiddenRunicEnchantsRequired <= 0) && rogue.armor->hiddenRunic == A_REFLECTION)) && netEnchant(rogue.armor) > 0)
					&& !(target->info.flags & MONST_REFLECT_4)
					&& (alwaysUse || rand_percent(50))) {
					if (canDirectlySeeMonster(monst)) {
						monsterName(monstName, monst, true);
					...
					zap(originLoc, targetLoc, BOLT_NEGATION, 10, 1.0, 0L);
					return true;
				}
				...
				if ((monst->info.abilityFlags & MA_CAST_SLOW)
					...
					zap(originLoc, targetLoc, BOLT_SLOW, 10, 1.0, 0L);
					return true;
				}

				// Sentry.
				if ((monst->info.abilityFlags & MA_CAST_SENTRY)
					&& !(target->info.flags & MONST_INANIMATE)
                    && (alwaysUse || rand_percent(35))) {
                    
					if (canDirectlySeeMonster(monst)) {
						monsterName(monstName, monst, true);
						sprintf(buf, "%s summons a sentry", monstName);
						combatMessage(buf, 0);
					}
					zap(originLoc, targetLoc, BOLT_SENTRY, 10, 1.0, 0L);
					return true;
				}
		...
		CYCLE_MONSTERS_AND_PLAYERS(target) {
			if (target != monst
				&& ((target->status[STATUS_ENTRANCED] && target->creatureState != MONSTER_ALLY)
					|| ((target->status[STATUS_POLYMORPHED] || target->status[STATUS_DOMINATED]) && monst->creatureState != MONSTER_ALLY)
	...
	if (!monst->status[STATUS_CONFUSED]
		...
		CYCLE_MONSTERS_AND_PLAYERS(target) {
			if (!abortHaste
				...
				targetLoc[0] = target->xLoc;
				targetLoc[1] = target->yLoc;
				zap(originLoc, targetLoc, BOLT_HASTE, 2, 1.0, 0L);
				return true;
			}
		}
	}
	...
	if (!monst->status[STATUS_CONFUSED]
		...
		CYCLE_MONSTERS_AND_PLAYERS(target) {
			if (!abortHaste
				...
				targetLoc[0] = target->xLoc;
				targetLoc[1] = target->yLoc;
				zap(originLoc, targetLoc, BOLT_SHIELDING, (monst->creatureState == MONSTER_ALLY ? 3 : 5), 1.0, 0L);
				return true;
			}
		}
	}
	...
	if (!monst->status[STATUS_CONFUSED]
        && (monst->info.abilityFlags & MA_CAST_INVISIBILITY)
        && (alwaysUse || rand_percent(50))) {
		...
		CYCLE_MONSTERS_AND_PLAYERS(target) {
			if (!abortHaste
				&& target != monst
				&& (!target->status[STATUS_INVISIBLE] || target->status[STATUS_INVISIBLE] * 2 < target->maxStatus[STATUS_INVISIBLE]) // wait, what? probably don't need this
				&& (target->creatureState == MONSTER_TRACKING_SCENT
					|| target->creatureState == MONSTER_FLEEING
					|| target->creatureState == MONSTER_ALLY
					|| target == &player)
				...
				if (canDirectlySeeMonster(monst)) {
					monsterName(monstName, monst, true);
					sprintf(buf, "%s casts a concealing spell", monstName);
					combatMessage(buf, 0);
				}
				
				targetLoc[0] = target->xLoc;
				targetLoc[1] = target->yLoc;
				zap(originLoc, targetLoc, BOLT_INVISIBILITY, 3, 1.0, 0L);
				return true;
			}
		}
	}	
	...
	if (!monst->status[STATUS_CONFUSED]
		...
		if (weakestAllyHealthFraction < 100) {
			...
			zap(originLoc, targetLoc, BOLT_HEALING, (monst->creatureState == MONSTER_ALLY ? 2 : 5), 1.0, 0L);
			return true;
		}
	}
	...
	if (!monst->status[STATUS_CONFUSED] && monst->info.abilityFlags & (MA_CAST_FIRE | MA_CAST_SPARK | MA_CALL_GUARDIAN)) {
		CYCLE_MONSTERS_AND_PLAYERS(target) {
			if (monstersAreEnemies(monst, target)
				&& monst != target
				&& !(monst->bookkeepingFlags & MONST_SUBMERGED)
                ...
                if (monst->info.abilityFlags & (MA_CAST_FIRE)
                    ...
                    if (!(burnedTerrainFlagsAtLoc(listOfCoordinates[0][0], listOfCoordinates[0][1]) & T_IS_FIRE)
                        ...
                        zap(originLoc, targetLoc, BOLT_FIRE, 4, 1.0, 0L);
                        if (player.currentHP <= 0) {
                ...
                if ((monst->info.abilityFlags & MA_CAST_SPARK)
                    ...
                    zap(originLoc, targetLoc, BOLT_LIGHTNING, 1, 1.0, 0L);
                    if (player.currentHP <= 0) {
                        monsterName(monstName, monst, false);
                        gameOver(monsterCatalog[monst->info.monsterID].monsterName, false);
                    }
				...
				if ((monst->info.abilityFlags & MA_CALL_GUARDIAN)
					&& (target == &player)
					&& (pmap[player.xLoc][player.yLoc].flags & (IS_IN_MACHINE))
                    && (alwaysUse || rand_percent(35))) {
                    
                    if (canDirectlySeeMonster(monst)) {
                        monsterName(monstName, monst, true);
                        sprintf(buf, "%s calls out to nearby guardians", monstName);
                        combatMessage(buf, 0);
                    }
					
					activateMonstersinMachine(pmap[player.xLoc][player.yLoc].machineNumber, true);
                    return true;
                }
...
boolean allyFlees(creature *ally, creature *closestEnemy) {
    ...
    if (distanceDiagonal(x, y, closestEnemy->xLoc, closestEnemy->yLoc) < 10
    ...
    if (distanceDiagonal(x, y, closestEnemy->xLoc, closestEnemy->yLoc) < 4
...
void moveAlly(creature *monst) {
	...
	if (cellHasTerrainFlag(x, y, (T_HARMFUL_TERRAIN & ~(T_IS_FIRE | T_CAUSES_DAMAGE | T_CAUSES_FIERY_DAMAGE | T_CAUSES_PARALYSIS | T_CAUSES_CONFUSION)))
		|| (cellHasTerrainFlag(x, y, T_IS_FIRE) && !monst->status[STATUS_IMMUNE_TO_FIRE])
		|| (cellHasTerrainFlag(x, y, T_CAUSES_FIERY_DAMAGE) && !(monst->info.flags & MONST_FIERY))
	...
	for (target = monsters->nextCreature; target != NULL; target = target->nextCreature) {
		if (target != monst
			...
			&& !(target->info.flags & MONST_IMMUNE_TO_WEAPONS)
			&& (!target->status[STATUS_ENTRANCED] || target->status[STATUS_ENTRANCED_BY_WAND])			
			&& distanceDiagonal(x, y, target->xLoc, target->yLoc) < shortestDistance
			...
			shortestDistance = distanceDiagonal(x, y, target->xLoc, target->yLoc);
	...
	if (closestMonster
		&& (distanceDiagonal(x, y, player.xLoc, player.yLoc) < leashLength || (monst->bookkeepingFlags & MONST_DOES_NOT_TRACK_LEADER))
		&& !(monst->info.flags & MONST_MAINTAINS_DISTANCE)) {
		...
		if (monst->xLoc == monst->targetCorpseLoc[0]
			...
			monst->bookkeepingFlags |= MONST_ABSORBING;
			   ...
			   || (distanceDiagonal(x, y, player.xLoc, player.yLoc) < 3 && (pmap[x][y].flags & IN_FIELD_OF_VIEW))) {
		...
		if ((monst->info.abilityFlags & MA_CAST_BLINK)
			&& !(monst->bookkeepingFlags & MONST_GIVEN_UP_ON_SCENT)
			&& distanceDiagonal(x, y, player.xLoc, player.yLoc) > 10
...
void monstersTurn(creature *monst) {
	...
	// Allow smart assassins to get away with murder; others to get in trouble
	if (monst->bookkeepingFlags & MONST_ASSASSINATION_ALARM && ! (monst->status[STATUS_HALLUCINATING] 
																  || monst->status[STATUS_SLOWED]
																  || monst->status[STATUS_CONFUSED] 
																  || monst->status[STATUS_BURNING] 
																  || monst->status[STATUS_POISONED] 
																  || monst->status[STATUS_STUCK] 
																  || monst->status[STATUS_NAUSEOUS]
																  || monst->status[STATUS_DISCORDANT]
																  || monst->status[STATUS_MAGICAL_FEAR]
																  || monst->status[STATUS_DARKNESS]
																  || monst->status[STATUS_AGGRAVATED])) {
		monsterName(buf2, monst, true);
		sprintf(buf, "%s shrieks 'Assassin!'.", buf2);
		...
		monst->bookkeepingFlags &= ~(MONST_ASSASSINATION_ALARM);
		monst->ticksUntilTurn = monst->movementSpeed;
		return;
	}
	...
	if ((monst->creatureState == MONSTER_TRACKING_SCENT
		...
		if (distanceDiagonal(x, y, player.xLoc, player.yLoc) > 1
			|| diagonalBlocked(x, y, player.xLoc, player.yLoc)) {
			for (ally = monsters->nextCreature; ally != NULL; ally = ally->nextCreature) {
				if (monstersAreEnemies(monst, ally) && distanceDiagonal(x, y, ally->xLoc, ally->yLoc) == 1
		...
		if ((monst->status[STATUS_LEVITATING] || monst->info.flags & MONST_RESTRICTED_TO_LIQUID || monst->bookkeepingFlags & MONST_SUBMERGED
			...
			&& (distanceDiagonal(x, y, player.xLoc, player.yLoc)) / 100 < monst->info.sightRadius
			&& pmap[x][y].flags & IN_FIELD_OF_VIEW) {
		...
		if (pmap[x][y].flags & IN_FIELD_OF_VIEW) {
			...
			dir = nextStep(safetyMap, monst->xLoc, monst->yLoc, monst, true);
		} else {
			if (!monst->safetyMap) {
				monst->safetyMap = allocDynamicGrid();
				copyDynamicGrid(monst->safetyMap, safetyMap);
			}
			dir = nextStep(monst->safetyMap, monst->xLoc, monst->yLoc, monst, true);
		}
		if (dir != -1) {
			targetLoc[0] = x + nbDirs[dir][0];
			targetLoc[1] = y + nbDirs[dir][1];
		...
		if (dir == -1 || (!moveMonster(monst, nbDirs[dir][0], nbDirs[dir][1]) && !moveMonsterPassivelyTowards(monst, targetLoc, true))) {
			CYCLE_MONSTERS_AND_PLAYERS(ally) {
				if (!monst->status[STATUS_MAGICAL_FEAR] // Fearful monsters will never attack.
					...
					&& distanceDiagonal(x, y, ally->xLoc, ally->yLoc) <= 1) {
		...
		// if a captive leader is adjacent and captive and healthy enough to withstand an attack
		// and regenerates damage, then attack him.
		if (monst->bookkeepingFlags & MONST_FOLLOWER && monst->leader->bookkeepingFlags & MONST_CAPTIVE
			&& distanceDiagonal(monst->xLoc, monst->yLoc, monst->leader->xLoc, monst->leader->yLoc) == 1
			...
			attack(monst, monst->leader, false, false);
			monst->ticksUntilTurn = monst->attackSpeed;
			return;
		}
...
//					&& !diagonalBlocked(monst->xLoc, monst->yLoc, ally->xLoc, ally->yLoc)
		...
		if ((monst->bookkeepingFlags & MONST_FOLLOWER) && distanceDiagonal(x, y, monst->leader->xLoc, monst->leader->yLoc) > 2) {
			moveTowardLeader(monst);
		} else {
		...
		if (monst->bookkeepingFlags & MONST_SEIZED) {
			for (defender = monsters->nextCreature; defender != NULL; defender = defender->nextCreature) {
				if ((defender->bookkeepingFlags & MONST_SEIZING)
					&& monstersAreEnemies(monst, defender)
					&& distanceDiagonal(monst->xLoc, monst->yLoc, defender->xLoc, defender->yLoc) == 1
	...
	if (((defender && (defender->info.flags & MONST_ATTACKABLE_THRU_WALLS))
		...
		&& (!defender || monst->status[STATUS_CONFUSED] || monst->status[STATUS_ENTRANCED] ||
			...
			if (defender) {
				...
				hitCount = makeHitList(hitList, monst->info.abilityFlags, newX, newY, tDirs[newX-monst->xLoc+1][newY-monst->yLoc+1]);
				// Attack!
				for (i=0; i<hitCount; i++) {
					if (hitList[i]
						&& monstersAreEnemies(monst, hitList[i])
						&& !(hitList[i]->bookkeepingFlags & MONST_IS_DYING)) {
						attack(monst, hitList[i], false, false);
					}
				}
				...
				monst->ticksUntilTurn = monst->movementSpeed;
				refreshDungeonCell(x, y);
				if (monst->creatureState != MONSTER_ALLY && rogue.impaleDirection >= 0
					&& newX == (player.xLoc + nbDirs[rogue.impaleDirection][0]) && newY == (player.yLoc + nbDirs[rogue.impaleDirection][1])) {
					rogue.impaleDirection = NO_DIRECTION;
					refreshDungeonCell(newX, newY);
					if (attack(&player, monst, false, true)) {
						if (monst->bookkeepingFlags & MONST_IS_DYING) {
							rogue.impaleDirection = tDirs[newX-player.xLoc+1][newY-player.yLoc+1];
							if (monst->info.bloodType && dungeonFeatureCatalog[monst->info.bloodType].tile && tileCatalog[dungeonFeatureCatalog[monst->info.bloodType].tile].foreColor) {
								rogue.weaponBloodstainColor = *tileCatalog[dungeonFeatureCatalog[monst->info.bloodType].tile].foreColor;
							}
							bakeColor(&rogue.weaponBloodstainColor);
							refreshDungeonCell(newX, newY);
							return true;
						}
					}
...
void monsterDetails(char buf[], creature *monst) {
	...
	short i, j, combatMath, combatMath2, playerKnownAverageDamage, playerKnownMaxDamage, commaCount, realArmorValue, realVorpalEnemy, range;
	...
	if (!(monst->info.flags & MONST_ATTACKABLE_THRU_WALLS)
		...
		sprintf(newText, "%s is trapped %s %s.\n     ",
				capMonstName,
				(tileCatalog[pmap[monst->xLoc][monst->yLoc].layers[layerWithFlag(monst->xLoc, monst->yLoc, T_OBSTRUCTS_PASSABILITY)]].flags & T_STAND_IN_TILE) ? "in" : "on",
	...
	if ((monst->info.flags & MONST_RESTRICTED_TO_LIQUID) && !cellHasTerrainFlag(monst->xLoc, monst->yLoc, T_ALLOWS_SUBMERGING)) {
		sprintf(newText, "     %s writhes helplessly on dry land.\n     ", capMonstName);
			   ...
			   && rogue.armor->vorpalEnemy == monst->info.slayID) {
		...
		if (rogue.shield) {
			if (monst->info.abilityFlags & MA_ATTACKS_FROM_DISTANCE) {
				range = distanceDiagonal(player.xLoc, player.yLoc, monst->xLoc, monst->yLoc) - 1;
			}
			
			if (rogue.shield->flags & ITEM_IDENTIFIED) {
				// Reduce chance of hitting by shield percentage
				combatMath2 = combatMath2 * (100 - clamp(rogue.shield->shieldChance + netEnchant(rogue.shield) * 4 + 3 * range, 0, min(100, 95 + range))) / 100;				
			} else {
				// Reduce chance of hitting by shield percentage
				combatMath2 = combatMath2 * (100 - clamp(rogue.shield->shieldChance + strengthModifier(rogue.shield) * 4 + 3 * range, 0, min(100, 95 + range))) / 100;
			}
		}
		
		sprintf(newText, "%s has a %i%% chance to hit you%s, typically hits for %i%% of your current health, and at worst, could defeat you in %i hit%s.\n     ",
				capMonstName,
				combatMath2,
				range ? " at this range" : "",
	...
	if (monst->creatureState == MONSTER_ALLY) {
		...
		if (rogue.weapon && rogue.weapon->flags & ITEM_RUNIC_IDENTIFIED && ((rogue.weapon->flags & ITEM_RUNIC && rogue.weapon->enchant2 == W_SLAYING)
			|| (rogue.talisman && rogue.talisman->kind == TALISMAN_RUNE_MASTERY
				&& (rogue.weapon->hiddenRunicEnchantsRequired <= 0) && rogue.weapon->hiddenRunic == W_SLAYING))
			&& rogue.weapon->vorpalEnemy == monst->info.slayID) {
			playerKnownAverageDamage = monst->info.maxHP;
			playerKnownMaxDamage = monst->info.maxHP;
		} else if (!rogue.weapon || (rogue.weapon->flags & ITEM_IDENTIFIED)) {
		...
		if (playerKnownMaxDamage == 0) {
			...
			if (rogue.weapon && !(rogue.weapon->flags & ITEM_IDENTIFIED)) {
				...
				realVorpalEnemy = rogue.weapon->vorpalEnemy;
				rogue.weapon->enchant1 = 0;
				rogue.weapon->vorpalEnemy = 0;
				combatMath2 = hitProbability(&player, monst);
				rogue.weapon->enchant1 = realArmorValue;
				rogue.weapon->vorpalEnemy = realVorpalEnemy;				
			} else if (rogue.weapon && !(rogue.weapon->flags & ITEM_RUNIC_IDENTIFIED)) {
				realVorpalEnemy = rogue.weapon->vorpalEnemy;
				rogue.weapon->vorpalEnemy = 0;
				combatMath2 = hitProbability(&player, monst);
				rogue.weapon->vorpalEnemy = realVorpalEnemy;				
			...
			if (monst->info.flags & MONST_SHIELD_BLOCKS) {
				if (rogue.weapon && rogue.weapon->kind >= DART) {
					range = distanceDiagonal(player.xLoc, player.yLoc, monst->xLoc, monst->yLoc) - 1;
				}
				
				// Reduce chance of hitting by shield percentage
				combatMath2 = combatMath2 * (100 - clamp(monst->info.defense / 2 + 3 * range, 0, min(100, 95 + range))) / 100;
			}
			
			sprintf(newText, "You have a %i%% chance to hit %s%s, typically hit for %i%% of $HISHER current health, and at best, could defeat $HIMHER in %i hit%s.",
					combatMath2,
					monstName,
					range ? " at this range" : "",
		...
		if ((theItem->category == STAFF || theItem->category == WAND)
			...
			switch (theItem->kind) {
				...
				case BOLT_LIGHTNING:
					if (monst->info.flags & MONST_SHIELD_BLOCKS) {
						range = distanceDiagonal(player.xLoc, player.yLoc, monst->xLoc, monst->yLoc) - 1;
						sprintf(newText, "\n     Your %s (%c) will hit %s for between %i%% and %i%% of $HISHER current health %i%% of the time%s%s.",
								theItemName,
								theItem->inventoryLetter,
								monstName,
								100 * boltDamageLow(theItem->category == STAFF ? theItem->enchant1 : 10) / monst->currentHP,
								100 * boltDamageHigh(theItem->category == STAFF ? theItem->enchant1 : 10) / monst->currentHP,
								100 - clamp(monst->info.defense/2 + 3 * range, 0, min(100, 95 + range)),
								range ? " at this range" : "",
								theItem->kind == BOLT_FORCE ? "if it collides with an obstruction, tripled if a wall" : "");
					} else {	
						sprintf(newText, "\n     Your %s (%c) will hit %s for between %i%% and %i%% of $HISHER current health%s.",
								theItemName,
								theItem->inventoryLetter,
								monstName,
								100 * boltDamageLow(theItem->category == STAFF ? theItem->enchant1 : 10) / monst->currentHP,
				...
				case BOLT_POISON:
					if (monst->info.flags & MONST_INANIMATE) {
						...
						sprintf(newText, "\n     Your %s (%c) will poison %s for %i%% of $HISHER current health.",
								...
								theItem->inventoryLetter,
								monstName,
								100 * boltPoison(theItem->category == STAFF ? theItem->enchant1 : 10) / monst->currentHP);
					}
					anyFlags = true;
					break;
				case BOLT_DOMINATION:
					if (!printedDominationText) {
						printedDominationText = true;
						if (monst->info.flags & MONST_INANIMATE) {
							sprintf(newText, "\n     A %s of domination will have no effect on inanimate objects like %s.",
									theItem->category == STAFF ? "staff" : "wand",
									monstName);
						} else if (theItem->category == STAFF && monst->creatureState != MONSTER_TRACKING_SCENT) {
							sprintf(newText, "\n     A staff of domination will always dominate %s who is not hunting you.",
									monstName);
						} else {
							sprintf(newText, "\n     A %s of domination will have a %i%% chance of success at %s's current health level.",
									theItem->category == STAFF ? "staff" : "wand",
									boltDominate(monst),
									monstName);
						}
