...
void updateMinersLightRadius() {
	...
	if (player.status[STATUS_LIGHTENED]) {
		lightRadius = lightRadius * 2 + 5;
	}
...
void updateLighting() {
	...
	CYCLE_MONSTERS_AND_PLAYERS(monst) {	
		...
		if (monst->status[STATUS_REFLECTIVE]) {
			paintLight(&lightCatalog[REFLECTIVE_CREATURE_LIGHT], monst->xLoc, monst->yLoc, false, false);
		}
		
		if (monst->status[STATUS_MARKED]) {
			paintLight(&lightCatalog[MARKED_LIGHT], monst->xLoc, monst->yLoc, false, true);
		}
		
		// Don't paint telepathy over marked monster lights
		else if (player.status[STATUS_TELEPATHIC] && &player != monst && !(monst->info.flags & MONST_INANIMATE)) {
