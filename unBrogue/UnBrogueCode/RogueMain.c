...
void initializeRogue(unsigned long seed) {
	...
	// reset wand and staff frequencies
	for (i=0;i<NUMBER_BOLT_KINDS;i++) {
		staffTable[i].frequency = boltTable[i].frequency;
		if (i < BOLT_SENTRY) {
			wandTable[i].frequency = 10;
		} else if (i == BOLT_SENTRY) {
			wandTable[i].frequency = 5;
		} else if (i == BOLT_BLOAT) {
			wandTable[i].frequency = 2;
		} else {
			wandTable[i].frequency = 1;
		}
	}
	...
	for (i=0; i<101; i++) {
		...
		} while (distanceDiagonal(levels[i].upStairsLoc[0], levels[i].upStairsLoc[1],
	...
	for (i=0; i<NUMBER_MONSTER_KINDS; i++) {
		...
		if (!monsterCatalog[i].slayID) {
			monsterCatalog[i].slayID = i;
		}
	}
	
	shuffleFlavors();
	
	// ensure at least one good attack staff
	i = rand_range(BOLT_FORCE, BOLT_POISON); // was BOLT_FORCE, BOLT_CONJURATION
	wandTable[i].frequency = 0;
	
	// ensure at least one good attack wand
	//j = rand_range(BOLT_FORCE, BOLT_CONJURATION - 1);
	//if (j >= i) j++;
	//staffTable[j].frequency = 0;	
	
	// ensure at least one good nonattack staff
	i = rand_range(BOLT_TELEPORT, BOLT_OBSTRUCTION);
	wandTable[i].frequency = 0;
	
	// ensure at least one good nonattack wand
	j = rand_range(BOLT_TELEPORT, BOLT_OBSTRUCTION - 1);
	if (j >= i) j++;
	staffTable[j].frequency = 0;
	
	// ensure at least one bad staff
	i = rand_range(BOLT_POLYMORPH, BOLT_SENTRY);
	wandTable[i].frequency = 0;
	
	// ensure at least one bad wand
	j = rand_range(BOLT_POLYMORPH, BOLT_SENTRY - 1);
	if (j >= i) j++;
	staffTable[j].frequency = 0;
	
	// assign whether remaining bolt items are wands or staffs
	for (i=0;i<NUMBER_BOLT_KINDS;i++) {
		if (!staffTable[i].frequency || !wandTable[i].frequency) {
			continue;
		} else if (rand_percent(i < BOLT_SENTRY ? 33 : 16)) {
			staffTable[i].frequency = 0;
		} else {
			wandTable[i].frequency = 0;
		}
	}
	...
	rogue.talisman = NULL;
	rogue.talismanFraction = 0;
	rogue.assassinations = 0;
	rogue.assassinRewards = 0;
	rogue.markedMonsters = 0;
	...
	= rogue.awarenessBonus = rogue.stealthBonus = rogue.wisdomBonus
	...
	theItem->hiddenRunic = rand_range(0, NUMBER_GOOD_WEAPON_ENCHANT_KINDS - 1); // required because we may have just cancelled a real runic
	theItem->hiddenRunicEnchantsRequired = rand_range(3, 6);
	identify(theItem);
	theItem = addItemToPack(theItem);
	...
	theItem->enchant1 = 0;
	theItem->flags &= ~(ITEM_CURSED | ITEM_RUNIC);
	theItem->hiddenRunic = rand_range(0, NUMBER_GOOD_ARMOR_ENCHANT_KINDS - 1); // required because we may have just cancelled a real runic
	theItem->hiddenRunicEnchantsRequired = rand_range(3, 6);
	identify(theItem);
	theItem = addItemToPack(theItem);
	equipItem(theItem, false);
...
void startLevel(short oldLevelNumber, short stairDirection) {
	...
	if (oldLevelNumber != rogue.depthLevel) {
		...
		for (flying = 0; flying <= 1; flying++) {
			fillDynamicGrid(mapToStairs, 0);
			calculateDistances(mapToStairs, px, py, (flying ? T_OBSTRUCTS_PASSABILITY : T_PATHING_BLOCKER), NULL, false, true, true);
	...
	rogue.impaleDirection = NO_DIRECTION; // prevent impaling duration simulation, also reset it when entering a new level
	...
	if (!levels[rogue.depthLevel - 1].visited) { // level has not already been visited
		...
		calculateDistances(mapToPit, levels[rogue.depthLevel-1].playerExitedVia[0],
						   levels[rogue.depthLevel-1].playerExitedVia[0], T_PATHING_BLOCKER, NULL, false, true, true);
		for (monst = monsters->nextCreature; monst != NULL; monst = monst->nextCreature) {
	...
	if (stairDirection == 0) { // fell into the level
		...
		if (!placedPlayer) {
            getQualifyingLocNear(loc, player.xLoc, player.yLoc, true, 0,
                                 (T_PATHING_BLOCKER),
                                 (HAS_MONSTER | HAS_ITEM | HAS_STAIRS | IS_IN_MACHINE), false, true);
        }
	}
	player.xLoc = loc[0];
	player.yLoc = loc[1];
	
	// Place new assassination rewards
	if (rogue.assassinations > rogue.assassinRewards) {
		...
		if (getQualifyingLocNear(loc, loc[0], loc[1], false, 0,
								 (T_PATHING_BLOCKER),
								 (HAS_MONSTER | HAS_ITEM | HAS_STAIRS | IS_IN_MACHINE), false, false)) {
			switch((++rogue.assassinRewards) % 5) {
				case 0:
					theItem = generateItem(WEAPON, DART);
					break;
				case 2:
					theItem = generateItem(WEAPON, rand_range(INCENDIARY_DART, GRAPPLING_DART));
					break;
				default:
