...
void printInstructionsScreen() {
	...
	char helpText[BROGUE_HELP_LINE_COUNT][DCOLS*3] = {
		...
		"          -- Basic tips --",
		"",
		"  - ****Press the tab key to get information on a nearby creature or item.",
		"    ****Press it again to cycle through nearby creatures and items.",
		"",
		"  - ****Stealth is a core mechanic. Your stealth range is shown visually",
		"    ****in a radius around your character. Outside of this range, most",
        ...
        "",
		"  - ****You may exit the dungeon through the entrance at any time to end the",
		"    ****game, but leaving without the Amulet is hardly a victory.",
		"",
		"  - ****Most potions will have different effects when quaffed, thrown at",
		"    ****a monster, or thrown to the ground.",
		"",
		"        -- press space or click to continue --"
	};
...
void printDiscoveries(short category, short count, unsigned short itemCharacter, short x, short y, cellDisplayBuffer dbuf[COLS][ROWS]) {
	...
	short i, j = 0, magic, totalFrequency;
...
/*
		if (theTable[i].identified) {
			theColor = &white;
			plotCharToBuffer(itemCharacter, x, y + j, &itemColor, &black, dbuf);
		} else {
			theColor = &darkGray;
            magic = magicCharDiscoverySuffix(category, i);
            if (magic == 1) {
                plotCharToBuffer(GOOD_MAGIC_CHAR, x, y + j, &goodColor, &black, dbuf);
            } else if (magic == -1) {
                plotCharToBuffer(BAD_MAGIC_CHAR, x, y + j, &badColor, &black, dbuf);
            }
		}
*/
        ...
        if (magic == 1)
            ...
            plotCharToBuffer('-', x, y + j, &yellow, &black, dbuf);
		...
		upperCase(buf);
		strcat(buf, " ");
		printString(buf, x + 2, y + j, theColor, &black, dbuf);
		j++;
	}
}
...
//	printString("-- - CHARMS --", x = mapToWindowX(53), y = mapToWindowY(15), &flavorTextColor, &black, dbuf);
//	plotCharToBuffer(CHARM_CHAR, x + 3, y, &itemColor, &black, dbuf);
//	printDiscoveries(CHARM, NUMBER_CHARM_KINDS, CHARM_CHAR, x + 1, ++y, dbuf);
	...
	printString("-- RINGS --", mapToWindowX(30), y += NUMBER_POTION_KINDS + 1, &flavorTextColor, &black, dbuf);
	printDiscoveries(RING, NUMBER_RING_KINDS, RING_CHAR, mapToWindowX(31), ++y, dbuf);
	...
	printString("-- CHARMS --", mapToWindowX(53), y += NUMBER_STAFF_KINDS + 1, &flavorTextColor, &black, dbuf);
	printDiscoveries(CHARM, NUMBER_CHARM_KINDS, CHARM_CHAR, mapToWindowX(54), ++y, dbuf);
    */
...
short printMonsterInfo(creature *monst, short y, boolean dim, boolean highlight) {
	...
	if (monst->info.maxHP > 1
		...
		// If hallucinating, the health randomizes. Either way, we now print health.. -- gsr
		...
		&& (!(monst->info.flags & MONST_INANIMATE)
			...
			if (y < ROWS - 1) {
				if (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience && y < ROWS - 1) {
                    ...
                    if (monst->info.flags & MONST_DISTANT_FOLLOWER)
                        printString("     (Peaceful)     ", 0, y++, (dim ? &darkGray : &gray), &black, 0);
                    else
