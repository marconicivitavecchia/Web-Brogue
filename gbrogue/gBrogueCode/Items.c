...
item *makeItemInto(item *theItem, unsigned long itemCategory, short itemKind) {
	...
	switch (itemCategory) {
        ...
        case THROWING_WEAPON:
			if (itemKind < 0) {
				itemKind = chooseKind(throwingWeaponTable, NUMBER_THROWING_WEAPON_KINDS);
			}
			theEntry = &throwingWeaponTable[itemKind];
			theItem->damage = throwingWeaponTable[itemKind].range;
			theItem->strengthRequired = throwingWeaponTable[itemKind].strengthRequired;
			theItem->displayChar = THROWING_WEAPON_CHAR;
			theEntry = &throwingWeaponTable[itemKind];
			// Comes in stacks
...
//			theItem->quantity = rand_range(50 + rogue.depthLevel * 10, 100 + rogue.depthLevel * 15);
			theItem->quantity = 3 * rand_range(50 + rogue.depthLevel * 10, 100 + rogue.depthLevel * 15); // gsr
			break;
		case AMULET:
			theEntry = NULL;
...
item *placeItem(item *theItem, short x, short y) {
	...
	if (cellHasTerrainFlag(x, y, T_IS_DF_TRAP)
		...
		if (playerCanSee(x, y)) {
			...
			message(buf, false);//, true); annoying when a confused monster steps on a confusion trap like every other turn -- gsr
		}
...
//	if (theItem->category & (FOOD|POTION|SCROLL|GEM)) {
	...
	else if (theItem->category & THROWING_WEAPON && theItem->quiverNumber > 0) {
		for (tempItem = packItems->nextItem; tempItem != NULL; tempItem = tempItem->nextItem) {
			if (theItem->category == tempItem->category && theItem->kind == tempItem->kind
...
short numberOfItemsInPack() {
	...
	for(theItem = packItems->nextItem; theItem != NULL; theItem = theItem->nextItem) {
		theCount += (theItem->category & (WEAPON|THROWING_WEAPON) ? 1 : theItem->quantity);
	}
	return theCount;
}
...
void itemName(item *theItem, char *root, boolean includeDetails, boolean includeArticle, color *baseColor) {
	...
	cyanEscapeSequence[0] = '\0'; // for identified items -- gsr
	...
	if (baseColor) {
		...
		tempColor = teal;
		applyColorMultiplier(&tempColor, baseColor);
		encodeMessageColor(cyanEscapeSequence, 0, &tempColor);
	...
	switch (theItem -> category) {
		...
		case WEAPON:
			...
			break;
		case THROWING_WEAPON:
			sprintf(root, "%s%s", throwingWeaponTable[theItem->kind].name, pluralization);
			if (includeDetails) {
				sprintf(root, "%s%s <%i>", root, grayEscapeSequence, theItem->strengthRequired);
			}
			break;
		...
		case SCROLL:
			if (scrollTable[theItem->kind].identified || rogue.playbackOmniscience) {
				sprintf(root, "scroll%s of %s%s%s", pluralization, cyanEscapeSequence, scrollTable[theItem->kind].name, baseEscapeSequence);
		...
		case POTION:
			if (potionTable[theItem->kind].identified || rogue.playbackOmniscience) {
				sprintf(root, "potion%s of %s%s%s", pluralization, cyanEscapeSequence, potionTable[theItem->kind].name, baseEscapeSequence);
		...
		case STAFF:
			if (staffTable[theItem->kind].identified || rogue.playbackOmniscience) {
				sprintf(root, "staff%s of %s%s%s", pluralization, cyanEscapeSequence, staffTable[theItem->kind].name, baseEscapeSequence);
		...
		case RING:
			if (ringTable[theItem->kind].identified || rogue.playbackOmniscience) {
				sprintf(root, "ring%s of %s%s%s", pluralization, cyanEscapeSequence, ringTable[theItem->kind].name, baseEscapeSequence);
			...
			break;
		case CHARM:
            sprintf(root, "%s%s%s charm%s", cyanEscapeSequence, charmTable[theItem->kind].name, baseEscapeSequence, pluralization);
...
//			sprintf(root, "%slumenstone%s%s", yellowEscapeSequence, pluralization, baseEscapeSequence);
	...
	if (includeArticle) {
        ...
        if (theItem->quiverNumber != 0 && theItem->quantity > 1)
        {
            ...
            sprintf(article, "a bundle of %i ", theItem->quantity);
        }
        ...
        // .. except cloaks. They get an article. -- gsr
		else if ((theItem->category & ARMOR) && (theItem->kind == CLOAK))
           sprintf(article, "a%s ", (isVowelish(root) ? "n" : ""));
	}
...
itemTable *tableForItemCategory(enum itemCategory theCat, short *kindCount) {
	...
	switch (theCat) {
		...
		case WEAPON:
            ...
            break;
		case THROWING_WEAPON:
			returnedTable = throwingWeaponTable;
            returnedCount = NUMBER_THROWING_WEAPON_KINDS;
            break;
		case ARMOR:
...
float enchantIncrement(item *theItem) {
	if (theItem->category & (WEAPON | ARMOR | THROWING_WEAPON)) {
...
short effectiveRingEnchant(item *theItem) {
    ...
    {
        if (rogue.ringLeft && rogue.ringLeft->kind == RING_ENCHANTMENT)
            enchantment += effectiveRingEnchant(rogue.ringLeft);
        else if (rogue.ringRight && rogue.ringRight->kind == RING_ENCHANTMENT)
            enchantment += effectiveRingEnchant(rogue.ringRight);
	}
...
void itemDetails(char *buf, item *theItem) {
	...
	const char weaponRunicEffectDescriptions[NUMBER_WEAPON_RUNIC_KINDS][DCOLS] = {
	    "the enemy will be teleported elsewhere",
...
//        if (theItem->category == POTION && theItem->kind == POTION_LIFE) {
	...
	switch (theItem->category) {
        ...
        case THROWING_WEAPON:
            sprintf(buf2, "\n\nUpon a successful hit, %s%s normally deal%s between %i and %i points of direct damage to a target.",
                    (singular ? "a " : ""),
                    theName,
                    (singular ? "s" : ""),
                    theItem->damage.lowerBound,
            ...
            strcat(buf, buf2);
				...
				if (strengthModifier(theItem)) {
					sprintf(buf2, " However, the %s %s%s a %s%s%.2f%s because of your %s strength. ",
							theName,
			...
			if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
				...
				if (strengthModifier(theItem)) {
					sprintf(buf2, ", %s %s %s %s%s%.2f%s because of your %s strength. ",
							( (theItem->enchant1 || theItem->category & THROWING_WEAPON) ? "and" : "but"),
							(singular ? "carries" : "carry"),
						...
						if (theItem->enchant2 == W_SLAYING) {
							...
							if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
								switch (theItem->enchant2) {
									...
									case W_POISON:
										sprintf(buf2, " for %i turns. ",
												(int) (weaponPoisonDuration(enchant)));
										strcat(buf, buf2);
										nextLevelState = (int) (weaponPoisonDuration((float) (enchant + enchantIncrement(theItem))) + FLOAT_FUDGE);
										break;
/*									case W_SLOWING:
										sprintf(buf2, " for %i turns. ",
					...
					if ((theItem->flags & ITEM_RUNIC_IDENTIFIED) || rogue.playbackOmniscience) {
						...
						switch (theItem->enchant2) {
							...
							case A_TELEPORTATION:
								strcpy(buf2, "While worn, it will cause you to teleport to another spot on the level if it absorbs a blow. ");
								break;
                            case A_IMMOLATION:
		...
		case STAFF:
			...
			// for staffs that never recharge on their own -- gsr
			if (staffChargeDuration(theItem) == 0)
            {
                if ((theItem->flags & ITEM_IDENTIFIED)  || rogue.playbackOmniscience)
                {

                  sprintf(buf2, "\n\nThe %s has %i charges remaining out of a maximum of %i charges, and unlike other staffs, it does not recharge over time on its own. ",
                            theName,
                            theItem->charges,
                            theItem->enchant1);
                    strcat(buf, buf2);
                } else if (theItem->flags & ITEM_MAX_CHARGES_KNOWN) {
                    sprintf(buf2, "\n\nThe %s has a maximum of %i charges, and unlike other staffs, it does not recharge over time on its own. ",
                            theName,
                            theItem->enchant1);
                    strcat(buf, buf2);
                }
			...
			if (((theItem->flags & (ITEM_IDENTIFIED | ITEM_MAX_CHARGES_KNOWN)) && staffTable[theItem->kind].identified)
				...
				switch (theItem->kind) {
					case STAFF_LIGHTNING:
						sprintf(buf2, "The bolt from this staff deals damage to every creature in its line of fire, including after its bounce. (If the staff is enchanted, its average damage will increase by %i%%.)",
					...
					case STAFF_TUNNELING:
						sprintf(buf2, "The bolt from this staff will dissolve %i layers of obstruction. (If the staff is enchanted, this will increase to %i layers.)",
								staffTunnelDistance(theItem->enchant1),
								staffTunnelDistance(theItem->enchant1 + 1));
						break;
					case STAFF_BLINKING:
						...
						break;
					case STAFF_FORCE:
						sprintf(buf2, "This staff will fling a creature %i spaces farther away from you. (If the staff is enchanted, this will increase to %i spaces.)",
                                staffForceDistance(theItem->enchant1),
								staffForceDistance(theItem->enchant1 + 1));
						break;
...
/*					case STAFF_PROTECTION:
					...
					case STAFF_DOMINATION:
						sprintf(buf2, "This staff will turn a weakened enemy into an ally. ");
						break;
					default:
                ...
                if (theItem->enchant1) {
                    switch (theItem->kind) {
                        ...
                        case RING_WISDOM:
                            ...
                            break;
                        case RING_SPEED:
                            sprintf(buf2, "\n\nWhen worn, you will move and attack %i%% %s than your normal rate. (If the ring is enchanted, you will instead act %i%% %s.)",
                                    ringSpeedBonus(abs(theItem->enchant1)),
                                    (theItem->enchant1 >= 0 ? "faster" : "slower"),
                                    ringSpeedBonus(abs(theItem->enchant1 + 1)),
                                    (theItem->enchant1 >= 0 ? "faster" : "slower"));
                            strcat(buf, buf2);
                            break;
                        case RING_REAPING:
                            ...
                            strcat(buf, buf2);
                            break;
                        case RING_ENCHANTMENT:
                            sprintf(buf2, "\n\nSo long as you are wearing this ring, any ring worn on your other finger will be %s by %i. (If this ring is enchanted, your other ring's enchantment will %s by %i.)",
                                    (theItem->enchant1 >= 0 ? "enchanted" : "disenchanted"),
                                    abs(theItem->enchant1),
                                    (theItem->enchant1 >= 0 ? "increase" : "decrease"),
                                    abs(theItem->enchant1 + 1));
                            strcat(buf, buf2);
...
/*                case CHARM_NEGATION:
                    ...
                    break;*/
                case CHARM_IDENTIFY:
                    sprintf(buf2, "\n\nWhen used, the charm will allow you to identify one item and will recharge in %i turns. (If the charm is enchanted, it will recharge in %i turns.)",
                            charmRechargeDelay(theItem->kind, theItem->enchant1),
                            charmRechargeDelay(theItem->kind, theItem->enchant1 + 1));
                    break;
                case CHARM_DISCORD:
                    sprintf(buf2, "\n\nWhen used, the charm will cause discord among creatures in your field of view up to %i spaces away, and recharge in %i turns. (If the charm is enchanted, it will reach up to %i spaces and recharge in %i turns.)",
                            charmDiscordRadius(theItem->enchant1),
                            charmRechargeDelay(theItem->kind, theItem->enchant1),
...
char displayInventory(unsigned short categoryMask,
	...
	for (i=0; i<itemNumber; i++) {
		...
		if ((theItem->flags & ITEM_MAGIC_DETECTED)
			&& !(theItem->category & AMULET) && !(theItem->category & GEM)) { // Won't include food, keys, lumenstones or amulet.
	...
	//if (waitForAcknowledge) { // Always show number of items left -- gsr
		...
		extraLineCount++;
        if (waitForAcknowledge) { // moved waitForAcknowledge here -- gsr
...
void updateEncumbrance() {
	...
	moveSpeed = player.info.movementSpeed - ringSpeedBonus(rogue.speedBonus); // gsr
	attackSpeed = player.info.attackSpeed - ringSpeedBonus(rogue.speedBonus);
...
void rechargeItems(unsigned long categories) {
    ...
    for (tempItem = packItems->nextItem; tempItem != NULL; tempItem = tempItem->nextItem) {
        if (tempItem->category & categories & STAFF) {
            ...
            tempItem->enchant2 = (tempItem->kind == STAFF_BLINKING || tempItem->kind == STAFF_OBSTRUCTION || tempItem->kind == STAFF_OBSTRUCTION  ? 10000 : 5000) / tempItem->enchant1;
        }
...
//				magicWeaponHit(monst, theItem, false);
				magicWeaponHit(monst, theItem, false, damage);
			}
			messageWithColor(buf, messageColorFromVictim(monst), false);
		}
		...
		// (non-throwing) weapons don't break when thrown now -- gsr
			...
			switch (theItem->kind) {
				...
				case POTION_INVISIBILITY:
				    if (hitMonsterAtSpot && monst)
                    {
                        ...
                        sprintf(buf, "the flask shatters and %s suddenly disappears!", monstName);
                        message(buf, false);
                        hadEffect = true;
                    }
                    else
                ...
                case POTION_RESPIRATION:
				    if (hitMonsterAtSpot && monst)
                    {
                        ...
                        monsterName(monstName, monst, true);
                        monst->status[STATUS_UNBREATHING] = monst->maxStatus[STATUS_UNBREATHING] = 300;
                        sprintf(buf, "the flask shatters and a pocket of air surrounds %s.", monstName);
                        message(buf, false);
                    }
                    else
                    {
                        hadEffect = true;
                        strcpy(buf, "the flask shatters and a cleansing breeze envelops the area.");
                        colorFlash(&darkGray, 0, ANY_KIND_OF_VISIBLE, 2, 8, x, y);
                        message(buf, false);
                        for (i=0; i<DCOLS; i++)
                            for (j=0; j < DROWS; j++)
                                if ((x - i) * (x - i) + (y - j) * (y - j) <= 5 * 5)// gsr
                                    pmap[i][j].layers[GAS] = 0;
                    }
                ...
                case POTION_LEVITATION:
				    if (hitMonsterAtSpot && monst)
                    {
                        ...
                        if (!(monst->info.flags & MONST_FLIES))
                        {
                            monst->status[STATUS_LEVITATING] = monst->maxStatus[STATUS_LEVITATING] = 300;
				...
				case POTION_HEALING:
				    if (hitMonsterAtSpot && monst)
                    {
                        ...
                        monsterName(monstName, monst, true);
                        heal(monst, 100, true);
                        sprintf(buf, "the flask shatters and %s looks healthier.", monstName);
                        message(buf, false);
                    }
                    else
                    {
				...
				case POTION_HALLUCINATION:
				    if (hitMonsterAtSpot && monst)
                    {
                        if (polymorph(monst))
                        {
                            flashMonster(monst, boltCatalog[BOLT_POLYMORPH].backColor, 100);
                            monsterName(monstName, monst, true);
                            sprintf(buf, "the flask shatters and %s absorbs its contents, violently twisting it into a new form!", monstName);
                            message(buf, false);
                            hadEffect = true;
                        }
                    }
                    ...
                    {
                        strcpy(buf, "the flask shatters and a multi-hued gas billows out!");
                        message(buf, false);
...
/*			if (theItem->kind == POTION_HALLUCINATION && (theItem->flags & ITEM_MAGIC_DETECTED)) {
        ...
        // Wake up the monster if something's thrown at him -- seems obvious! -- gsr
        if (hitMonsterAtSpot && (monst = monsterAtLoc(x, y)) && monst->creatureState != MONSTER_ALLY && monst->creatureState == MONSTER_SLEEPING)
            alertMonster(monst);
...
//	maxDistance = max(4, ringPropulsionBonus(rogue.throwingBonus) + 0.75 * (rogue.strength - player.weaknessAmount - 12) + 2);
	...
	if (theItem->category & POTION && (magicCharDiscoverySuffix(POTION, theItem->kind) == 1) && potionTable[theItem->kind].identified)
        targetAllies = true;

	// Choose target
	if (chooseTarget(zapTarget, maxDistance, true, autoTarget, targetAllies, false, targetAllies ? &green : &red)) {
...
boolean useStaffOrWand(item *theItem, boolean *commandsRecorded) {
    ...
    // for more variation in tunneling -- gsr
    if (theItem->category == STAFF && theItem->kind == STAFF_TUNNELING)
        theBolt.magnitude = staffTunnelDistance(theItem->enchant1);
...
creature* summonMonster(short monsterKind) {
    short x = player.xLoc, y = player.yLoc;
    creature *monst;

    monst = generateMonster(monsterKind, false, false);
    getQualifyingPathLocNear(&(monst->xLoc), &(monst->yLoc), x, y, true,
                             avoidedFlagsForMonster(&(monst->info)) & ~T_SPONTANEOUSLY_IGNITES, HAS_PLAYER,
                             avoidedFlagsForMonster(&(monst->info)) & ~T_SPONTANEOUSLY_IGNITES, (HAS_PLAYER | HAS_MONSTER | HAS_UP_STAIRS | HAS_DOWN_STAIRS), false);
//    fadeInMonster(monst);
    pmap[monst->xLoc][monst->yLoc].flags |= HAS_MONSTER;
    return monst;
...
/*        case CHARM_NEGATION:
        ...
        case CHARM_IDENTIFY:
			if (numberOfMatchingPackItems(ALL_ITEMS, ITEM_CAN_BE_IDENTIFIED, 0, false) == 0) {
				message("Your charm flickers and fades. Everything in your pack is already identified.", false);
				break;
			}
			do {
...
void apply(item *theItem, boolean recordCommands) {
	...
	switch (theItem->category) {
		case FOOD:
			if (STOMACH_SIZE - player.status[STATUS_NUTRITION] < (theItem->kind == RATION ? NUTRITION_FOOD : NUTRITION_MANGO)) { // Not hungry enough.
		...
		case POTION:
			...
			break;
        case THROWING_WEAPON:
            throwCommand(theItem); // for accessibility
        ...
        case CHARM:
            ...
            useCharm(theItem);
            break;
        case WEAPON: // can apply to equip/unequip for accessibility -- gsr
        case ARMOR:
        case RING:
			if (theItem->flags & ITEM_EQUIPPED)
				unequip(theItem);
				...
				equip(theItem);
			return; // We essentially redirected to equip/unequip routines; we don't want to record our keystrokes here.
...
void readScroll(item *theItem) {
	...
	switch (theItem->kind) {
		...
		case SCROLL_PROTECT_EQUIPMENT:
			...
			if (!hadEffect)
				message("a protective golden light covers you, but it quickly disperses.", false);

/*
                for (tempItem = packItems->nextItem; tempItem != NULL; tempItem = tempItem->nextItem) {
                    if ((tempItem->category & (WEAPON|ARMOR)) && !(tempItem->flags & ITEM_PROTECTED)) {
                        i++;
                        tempItem->flags |= ITEM_PROTECTED;
				...
				message("a protective golden light covers your pack, but it quickly disperses.", false);
			}*/
...
/*		case SCROLL_PROTECT_ARMOR:
        ...
        case SCROLL_DISCORD:
            ...
            break;
		case SCROLL_GREAT_IDENTIFY:
			updateIdentifiableItems();
			if (!(theItem->flags & ITEM_IDENTIFIED))
                messageWithColor("this is a scroll of great identify.", &itemMessageColor, false);
			identify(theItem);
			if (numberOfMatchingPackItems(ALL_ITEMS, ITEM_CAN_BE_IDENTIFIED, 0, false) == 0) {
				message("everything in your pack is already identified.", false);
				break;
            ...
            message("You suddenly understand the functions of all of your items.", false); //gsr
...
/*		case POTION_DESCENT:
		...
		case POTION_LEVITATION:
			...
			break;
		case POTION_RESPIRATION:
			player.status[STATUS_UNBREATHING] = player.maxStatus[STATUS_UNBREATHING] = 100;
			message("A pocket of cool, clean air swirls around you.", false);
			break;
        case POTION_HEALING:
            message("you feel healed and refreshed from within!", false);
            heal(&player, 60, true);
            break;
		case POTION_TELEPATHY:
...
void dipItemIntoPotion(item *theItem) {
    ...
    if (!numberOfMatchingPackItems((POTION), 0, 0, false)) {
        confirmMessages();
        message("you have no potions to dip.", false);
        return;
    }
    do {
        thePotion = promptForItemOfType((POTION), 0, 0,
                                      KEYBOARD_LABELS ? "Dip into what? (a-z; shift for more info)" : "Dip into what?",
                                      false);
        confirmMessages();
        if (thePotion == NULL || !(thePotion->category & (POTION))) {
            message("Can't dip into that.", true);
        }
        if (rogue.gameHasEnded) {
            return;
        }
        if (thePotion == NULL) // nevermind
            return;
    } while (!(thePotion->category & (POTION)));
	...
	switch (thePotion->kind) {
		case POTION_INCINERATION:
			//colorFlash(&darkOrange, 0, IN_FIELD_OF_VIEW, 4, 4, player.xLoc, player.yLoc);
			message("you dip your thing into the unstable compound.", false);
			makeItemInto(theItem, THROWING_WEAPON, INCENDIARY_DART);
			break;
		case POTION_PARALYSIS:
			spawnDungeonFeature(player.xLoc, player.yLoc, &dungeonFeatureCatalog[DF_PARALYSIS_GAS_CLOUD_POTION], true, false);
...
uchar itemMagicChar(item *theItem) {
	switch (theItem->category) {
		...
		case ARMOR:
			...
			return 0;
			break;
        // Streamlined this and didn't break it... I hope..? --gsr
		case SCROLL:
        case POTION:
        case STAFF:
            return magicCharDiscoverySuffix(theItem->category, theItem->kind) == 1 ? GOOD_MAGIC_CHAR : BAD_MAGIC_CHAR;
		    /*
			switch (theItem->kind) {
				case SCROLL_AGGRAVATE_MONSTER:
				case SCROLL_SUMMON_MONSTER:
...
item *dropItem(item *theItem) {
	...
	if (theItem->quantity > 1 && !(theItem->category & (THROWING_WEAPON) )) { // peel off the top item and drop it
...
/*					break;
				...
				case RING_AWARENESS:
					...
					break;*/
				case RING_PERCEPTION: // gsr
					rogue.lightMultiplier += effectiveRingEnchant(rings[i]);
					rogue.awarenessBonus += 15 * effectiveRingEnchant(rings[i]);
					break;
                ...
                case RING_REAPING:
					...
					break;
                case RING_SPEED:
                    rogue.speedBonus += effectiveRingEnchant(rings[i]);
					break;
			}
		}
	}
...
//		750,	//W_CONFUSION,
	...
	switch (theItem->category) {
		...
		case WEAPON:
			...
			break;
        case THROWING_WEAPON:
			return throwingWeaponTable[theItem->kind].marketValue * theItem->quantity;
			break;
		case ARMOR:
...
void debugWish(char *wishText)
{
    ...
    // If we're passed in a wish, simply use that wish.
    if (strlen(wishText) > 0)
    {
        sprintf(buf, "%s", wishText);
        sprintf(buf2, "You wish \"%s\" again.", wishText);
        message(buf2, false);
    }
	...
	if (strlen(buf) != 0)
    {
        ...
        if (strstr(buf, "potion"))
        {
            for (i = 0; i < NUMBER_POTION_KINDS; i++)
            {
                if (strstr(buf, potionTable[i].name))
                {
                    theItem = generateItem(POTION, i);
                    success = true;
                }
            }
        }
        ...
        else if (strstr(buf, "scroll"))
        {
            for (i = 0; i < NUMBER_SCROLL_KINDS; i++)
            {
                if (strstr(buf, scrollTable[i].name))
                {
        ...
        else if (strstr(buf, "charm"))
        {
            ...
            {
                if (strstr(buf, charmTable[i].name))
                {
        ...
        else if (strstr(buf, "staff"))
        {
            for (i = 0; i < NUMBER_STAFF_KINDS; i++)
            {
                if (strstr(buf, staffTable[i].name))
                {
                    theItem = generateItem(STAFF, i);
                    theItem->enchant1 = 1;
        ...
        else if (strstr(buf, "ring"))
        {
            for (i = 0; i < NUMBER_RING_KINDS; i++)
            {
                if (strstr(buf, ringTable[i].name))
                {
                    theItem = generateItem(RING, i);
                    theItem->enchant1 = 1;
        ...
        // Weapons and armor could be pretty much any other string, so we'll handle them here.
        else
        {
            for (i = 0; i < NUMBER_WEAPON_KINDS; i++)
            {
                if (strstr(buf, weaponTable[i].name))
                {
            ...
            if (!success)
                ...
                {
                    if (strstr(buf, armorTable[i].name))
                    {
        ...
        if (success && theItem)
        {
            ...
            if ((theItem->category & (WEAPON | ARMOR | RING | CHARM | STAFF)))
            {
                if (buf[0] == '+' || buf[0] == '-')
                {
                    sscanf(buf,"%i", &enchantment);
                    theItem->enchant1 = enchantment;
                }
                else if (theItem->category & (WEAPON | ARMOR))
                    theItem->enchant1 = 0;
            }
            ...
            {
                theItem->enchant2 = 0;
                // Did we want a runic with that?
                for (i = 0; i < NUMBER_WEAPON_RUNIC_KINDS; i++)
                {
                    if (strstr(buf, weaponRunicNames[i]))
                    {
                        theItem->enchant2 = i;
                        theItem->flags |= ITEM_RUNIC;
                ...
                for (i = 0; i < NUMBER_ARMOR_ENCHANT_KINDS; i++)
                {
                    if (strstr(buf, armorRunicNames[i]))
                    {
                        theItem->enchant2 = i;
                        theItem->flags |= ITEM_RUNIC;
                        theItem->flags &= ~(ITEM_CURSED | ITEM_RUNIC_HINTED);
...
//            identify(theItem);
            ...
            message("Sorry, couldn't parse that.", false);
	}
	else
        confirmMessages();
    updateVision(true);
