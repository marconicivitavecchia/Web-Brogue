...
void startLevel(short oldLevelNumber, short stairDirection) {
		...
		for (flying = 0; flying <= 1; flying++) {
			...
			for (monst = monsters->nextCreature; monst != NULL; monst = monst->nextCreature) {
				...
				if (((monst->creatureState == MONSTER_TRACKING_SCENT && (stairDirection != 0 || monst->status[STATUS_LEVITATING]))
					 || monst->creatureState == MONSTER_ALLY || monst == rogue.yendorWarden || monst == rogue.moloch)
    ...
    if (!levels[rogue.depthLevel-1].visited) {
        ...
        // gsr
            ...
            messageWithColor("You feel a sense of dread as you delve deeper into the dungeon.", &pink, false);
    }
...
void gameOver(char *killedBy, boolean useCustomPhrasing) {
	...
	if (useCustomPhrasing) {
		...
		sprintf(highScoreText, "Killed by a%s %s on depth %i", (isVowelish(killedBy) ? "n" : ""), killedBy,
				rogue.depthLevel);
	}
...
// Also allow player to abandon the dungeon
void victory(boolean superVictory, boolean hasAmulet) {
    ...
    if (superVictory) {
        ...
        //message(    "You feel a sense of relief as you exit the dungeon for good, neglecting the Amulet of Yendor.", false);
        //copyDisplayBuffer(dbuf, displayBuffer);
        //funkyFade(dbuf, &black, 0, 5, mapToWindowX(player.xLoc), mapToWindowY(player.yLoc), false);
        //displayMoreSign();
        printString("You have abandoned the Dungeons of Doom.     ", mapToWindowX(0), mapToWindowY(-1), &white, &black, 0);
        displayMoreSign();
        clearDisplayBuffer(dbuf);
        deleteMessages();
        if (rogue.deepestLevel > 1)
            strcpy(displayedMessage[0], "You sell your meager treasures and return to your daily life.");
        else
            strcpy(displayedMessage[0], "You return in shame and sell your equipment.");
    }
	...
	if (!hasAmulet)
    {
/*        printString("Early termination fee", mapToWindowX(2), min(ROWS-1, i + 1), &white, &black, dbuf);
        sprintf(buf, "-%li", totalValue*19/20 + 500);
        printString(buf, mapToWindowX(60 - 1), min(ROWS-1, i + 1), &red, &black, dbuf);
        totalValue -= totalValue*19/20 + 500;*/
        i++;
    }
	i++;
	...
	if (totalValue < 0)
        printString(buf, mapToWindowX(60 - 1), min(ROWS-1, i + 1), &lightBlue, &black, dbuf);
    else
    ...
    if (hasAmulet)
        ...
        funkyFade(dbuf, &black, 0, 5, COLS/2, ROWS/2, true);

    strcpy(victoryVerb, superVictory ? "Mastered" : "Escaped");
	if (!hasAmulet)
    {
		if (rogue.deepestLevel == 1)
            sprintf(theEntry.description, "Had second thoughts about descending into the Dungeons.");
        else
            sprintf(theEntry.description, "Abandoned the Dungeons after descending to depth %i.", rogue.deepestLevel);
    }
	else if (gemCount == 0) {
